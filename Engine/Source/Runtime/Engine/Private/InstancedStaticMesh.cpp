...
static TAutoConsoleVariable<int32> CVarRayTracingVisualizeBVHShowISM(
	TEXT("r.RayTracing.VisualizeBVH.ShowFlags.InstancedStaticMesh"),
	1,
	TEXT("Include instanced static mesh bounds when visualizing BVH"));
...
#if RHI_RAYTRACING
		PerInstanceAABBs.Empty();
		PerInstanceAABBs.Reserve(InstanceCount);
#endif
	}
...
#if RHI_RAYTRACING
			PerInstanceAABBs.Add(TransformedBounds.GetBox());
#endif
		}
...
const TArray<FVector4>& FPerInstanceRenderData::GetPerInstanceBounds()
{
	if (!bTrackBounds)
	{
		// This might happen when changing things in the editor
		bTrackBounds = true;
	}
	EnsureInstanceDataUpdated();
...
#if RHI_RAYTRACING
const TArray<FBox>& FPerInstanceRenderData::GetPerInstanceAABBs()
{
	EnsureInstanceDataUpdated();
	return PerInstanceAABBs;
}
#endif
...
void FInstancedStaticMeshSceneProxy::GetDynamicRayTracingInstances(struct FRayTracingMaterialGatheringContext& Context, TArray<FRayTracingInstance>& OutRayTracingInstances)
{
	...
	const bool bGatherAABB = Context.bGatherAABB && CVarRayTracingVisualizeBVHShowISM->GetBool();
	...
	//NVRTX-TODO: We should just use FBoxSphere and keep one set of bounds, but this will make code merges more tedious.
	const TArray<FBox>& PerInstanceAABBs = InstancedRenderData.PerInstanceRenderData->GetPerInstanceAABBs();
		...
		if (CullAngle < 0.0f)
		{
			...
			if (PerInstanceTransforms.Num() > 0)
			{
				for (int InstanceIndex = 0; InstanceIndex < InstanceCount; InstanceIndex++)
				{
					...
					if (bGatherAABB)
					{
						FBox InstanceBounds = PerInstanceAABBs[InstanceIndex].TransformBy(GetLocalToWorld());
						RayTracingInstanceTemplate.InstanceAABBs.Add(InstanceBounds);
					}
				}
			}
		}
		else
		{
			...
			if (PerInstanceTransforms.Num() > 0)
			{
				for (int InstanceIndex = 0; InstanceIndex < InstanceCount; InstanceIndex++)
				{
					...
					{
						FMatrix InstanceToWorld = PerInstanceTransforms[InstanceIndex] * GetLocalToWorld();

						FBox InstanceAABB;
						if (bGatherAABB)
						{
							InstanceAABB = PerInstanceAABBs[InstanceIndex].TransformBy(GetLocalToWorld());
						}
						...
						if (bHasWorldPositionOffset)
						{
							...
							if (ActiveInstances[DynamicInstanceIdx] == -1)
							{
								...
								FMatrix DynamicInstanceToWorld = PerInstanceTransforms[DynamicInstanceIdx] * GetLocalToWorld();
								...
								Context.DynamicRayTracingGeometriesToUpdate.Add(
									FRayTracingDynamicGeometryUpdateParams
									{
										...
										true,
										DynamicInstanceIdx,
										DynamicInstanceToWorld.Inverse()
									}
								);
							}
							else
							...
							if (bGatherAABB)
							{
								DynamicInstance->InstanceAABBs.Add(InstanceAABB);
							}
						}
						else
						{
							...
							if (bGatherAABB)
							{
								RayTracingInstanceTemplate.InstanceAABBs.Emplace(InstanceAABB);
							}
						}
					}
				}
	...
	else
	{
		...
		if (PerInstanceTransforms.Num() > 0)
		{
			...
			{
				FMatrix InstanceToWorld = PerInstanceTransforms[InstanceIdx] * GetLocalToWorld();

				FBox InstanceAABB;
				if (bGatherAABB)
				{
					// This is still exported for visualization BVH
					InstanceAABB = PerInstanceAABBs[InstanceIdx].TransformBy(GetLocalToWorld());
				}
				...
				if (bHasWorldPositionOffset)
				{
					...
					const uint32 DynamicInstanceIdx = InstanceIdx % SimulatedInstances;
					...
					if (ActiveInstances[DynamicInstanceIdx] == -1)
					{
						...
						FMatrix& DynamicInstanceToWorld = InstanceToWorld;
						check(InstanceIdx == DynamicInstanceIdx);
				...
				{
					RayTracingInstanceTemplate.InstanceTransforms.Add(InstanceToWorld);

					if (bGatherAABB)
					{
						RayTracingInstanceTemplate.InstanceAABBs.Add(InstanceAABB);
					}
				}
			}
		}
	...
	{
		INC_DWORD_STAT_BY(STAT_RayTracingInstancedStaticMeshes, RayTracingInstanceTemplate.InstanceTransforms.Num());
...
bool FInstancedStaticMeshSceneProxy::UseRayTracingShadowBias() const
{
	//TODO: Even if instanced static meshes are evaluated for WPO, each instance is not exactly simulated yet, so we still need to apply the bias in this case.
	//const int32 WPOEvalMode = CVarRayTracingInstancesEvaluateWPO.GetValueOnRenderThread();
	//const bool bWantsWPOEvaluation = WPOEvalMode < 0 ? bDynamicRayTracingGeometry : WPOEvalMode != 0;

	static const auto UseShadowBiasForSkipWPOEvalCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.Shadow.UseBiasForSkipWPOEval"));
	static const auto UseAmbientOcclusionBiasForSkipWPOEvalCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.AmbientOcclusion.UseBiasForSkipWPOEval"));

	bool bUseBiasForSkipWPOEval = UseShadowBiasForSkipWPOEvalCVar->GetValueOnRenderThread() > 0 || UseAmbientOcclusionBiasForSkipWPOEvalCVar->GetValueOnRenderThread() > 0;

	return bUseBiasForSkipWPOEval /* && !bWantsWPOEvaluation */ && bAnySegmentUsesWorldPositionOffset;
}
