...
static TAutoConsoleVariable<int32> CVarRayTracingVisualizeBVHShowSkeletalMesh(
	TEXT("r.RayTracing.VisualizeBVH.ShowFlags.SkeletalMesh"),
	0,
	TEXT("Include skeletal mesh bounds when visualizing BVH"));
	...
	if (MeshObject->GetRayTracingGeometry() && MeshObject->GetRayTracingGeometry()->IsInitialized() )
	{
		...
		if (MeshObject->GetRayTracingGeometry()->RayTracingGeometryRHI.IsValid())
		{
			...
			if (LODIndex < SkeletalMeshRenderData->CurrentFirstLODIdx)
			{
				...
				if (Context.bGatherAABB && CVarRayTracingVisualizeBVHShowSkeletalMesh->GetBool())
				{
					RayTracingInstance.InstanceAABBs.Add(GetBounds().GetBox());
				}
