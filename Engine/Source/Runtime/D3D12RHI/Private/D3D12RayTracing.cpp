...
#if !PLATFORM_HOLOLENS && !PLATFORM_CPU_ARM_FAMILY && PLATFORM_WINDOWS
...
static int32 GRayTracingElevateASHeapPriority = 1;
static FAutoConsoleVariableRef CVarD3D12RayTracingElevateASHeapPriority(
	TEXT("r.D3D12.RayTracingElevateASHeapPriority"),
	GRayTracingElevateASHeapPriority,
	TEXT("Elevates the raytracing acceleration structure heap priority, which may help performance / stability in low memory conditions\n")
);
...
#if NV_API_ENABLE && RAY_TRACING_SUPPORT_TIMING
	if (GRHISupportsRayTracingShaderExtensions && IsRHIDeviceNVIDIA())
	{
		NvAPI_Status NvapiStatus = NvAPI_D3D12_SetNvShaderExtnSlotSpaceLocalThread(RayTracingDevice, 0, 1001);

		checkf(NvapiStatus == NVAPI_OK, TEXT("NVAPI ERROR %d on setup extensions\n"), NvapiStatus);
	}
#endif
	...
	const TCHAR* DebugName)
{
	...
	if (GRayTracingElevateASHeapPriority)
	{
