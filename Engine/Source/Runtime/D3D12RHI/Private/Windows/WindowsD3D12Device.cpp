...
#if RHI_RAYTRACING
			if (GetAdapter().GetD3DDevice5())
			{
				NvStatus = NvAPI_D3D12_SetNvShaderExtnSlotSpaceLocalThread(GetAdapter().GetD3DDevice5(), ~0u, 0);
				GRHISupportsRayTracingShaderExtensions = NvStatus == NVAPI_OK;

				if (NvStatus != NVAPI_OK)
				{
					UE_LOG(LogD3D12RHI, Warning, TEXT("Failed to query support for ray tracing extensions"));
				}
			}
#endif
		}
