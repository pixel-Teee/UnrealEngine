...
#if D3D12_RHI_RAYTRACING && PLATFORM_WINDOWS
	if (QBSS.RootSignatureType == RS_RayTracingGlobal)
	{
		//EHARTNV : Slot for extension UAVs
		check(RootParameterCount < MaxRootParameters);
		DescriptorRanges[RootParameterCount].Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0u, 1001, D3D12_DESCRIPTOR_RANGE_FLAG_NONE, 0);
