...
int32 GMaxRTLights = -1;
FAutoConsoleVariableRef CVarMaxRTLights(
	TEXT("r.RayTracing.Shadow.MaxLights"),
	GMaxRTLights,
	TEXT("Maximum number of ray traced shadow casting lights. (Default -1, any that are marked ray traced)"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

int32 GMaxDenoisedLights = -1;
FAutoConsoleVariableRef CVarMaxDenoisedLights(
	TEXT("r.RayTracing.Shadow.MaxDenoisedLights"),
	GMaxDenoisedLights,
	TEXT("Maximum number of ray traced shadow casting lights to denoise. (Default -1, any that are marked ray traced and desire denoising)"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

int32 GPrioritizeDirectional = 1;
FAutoConsoleVariableRef CVarPrioritizeDirectional(
	TEXT("r.RayTracing.Shadow.PrioritizeDirectional"),
	GPrioritizeDirectional,
	TEXT("Whether to count directional lights as close for the purposes of priority sorting (default 1)"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarFallbackToSharpShadows(
	TEXT("r.RayTracing.Shadow.FallbackToSharp"),
	0,
	TEXT("Whether to fallback to sharp shadows when denoising is not applied"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
bool ShouldRenderRayTracingShadowsForLight(const FLightSceneInfo& LightInfo)
{
	return ShouldRenderRayTracingShadows() && LightInfo.CastsRaytracedShadowThisFrame()
		&& ShouldRenderRayTracingShadowsForLightType((ELightComponentType)LightInfo.Proxy->GetLightType());
}


bool ShouldRenderRayTracingShadowsForLight(const FLightSceneInfoCompact& LightInfo)
{
	return ShouldRenderRayTracingShadows() && LightInfo.LightSceneInfo->CastsRaytracedShadowThisFrame()
...
FLightOcclusionType GetPreferredLightOcclusionType(const FLightSceneProxy& Proxy)
{
#if RHI_RAYTRACING
...
FLightOcclusionType GetActiveLightOcclusionType(const FLightSceneInfo& LightInfo)
{
#if RHI_RAYTRACING
...
FLightOcclusionType GetActiveLightOcclusionType(const FLightSceneInfoCompact& LightInfo)
{
#if RHI_RAYTRACING
	return ShouldRenderRayTracingShadowsForLight(LightInfo) ? FLightOcclusionType::Raytraced : FLightOcclusionType::Shadowmap;
#else
...
static bool LightRequiresDenosier(const FLightSceneInfo& LightSceneInfo)
{
	...
	if (!LightSceneInfo.DenoiseShadowThisFrame())
	{
		return false;
	}
...
// Temporary stat counter for tracking how many lights change whether or not they are ray tracing shadows each frame
DECLARE_DWORD_COUNTER_STAT(TEXT("Shadow Priority Changes"), STAT_ShadowPriorityChanges, STATGROUP_ShadowRendering)

#define RANKDEBUG 0

static void PrioritizeLights(FScene* Scene, const TArray<FViewInfo>& Views)
{
	// Iterate light list and select whether to use ray traced or other shadows for lights this frame
	if (IsRayTracingEnabled())
	{
		if (GMaxRTLights > -1 || GMaxDenoisedLights > -1)
		{
			// just base it on View 0 for now
			const FViewInfo& View = Views[0];
			const FVector ViewPosition = View.ViewLocation;

			struct FLightImportance
			{
				float Score = 0.f;
				int32 LightIndex = -1;
				uint8 bRequiresDynamicShadow = 0;
#if RANKDEBUG
				float Distance;
				float PerceptualLum;
				float PerceptualArea;
				float PenumbraMag;
				FLightShaderParameters LightParameters;
#endif // RANKDEBUG
			};

			TArray<FLightImportance> LightRanking;
			...
			for (TSparseArray<FLightSceneInfoCompact>::TIterator LightIt(Scene->Lights); LightIt; ++LightIt)
			{
				...
				// skip lights we're never going to render
				if (!LightSceneInfo->ShouldRenderLightViewIndependent())
				{
					continue;
				}

				if (LightSceneProxy->CastsRaytracedShadow())
				{
					...
					// Code derived from ShadowSetup.cpp
					// Determine whether this light would just be falling back if not for RT, these always will get shadows
					bool bNeedsDynamicShadow = false;
					{
						...
						bNeedsDynamicShadow = bPointLightWholeSceneShadow || bCreateShadowForMovableLight || bCreateShadowToPreviewStaticLight || bCreateShadowForOverflowStaticShadowing;
					}
					FLightImportance LightImportance;

					LightImportance.LightIndex = LightSceneInfo->Id;
					LightImportance.bRequiresDynamicShadow = bNeedsDynamicShadow;
					FLightShaderParameters LightParameters;
					...
					{
						PixelCount = ScissorRect.Area();
					}
					else
					{
						PixelCount = View.ViewRect.Area();
					}
					const float PerceptualArea = FMath::Sqrt(PixelCount);

					ELightComponentType LightType = ELightComponentType(LightSceneProxy->GetLightType());

					float PenumbraMag = 0.f;
					if (LightType == LightType_Point || LightType == LightType_Spot)
					{
						// sad hack^Wtrick to make worst-case penumbra from unbound source radius comparable with rect and directional light penumbra
						PenumbraMag = FMath::Atan(LightSceneInfo->Proxy->GetSourceRadius());
					}
					else if (LightType == LightType_Rect || (LightType == LightType_Directional && LightSceneProxy->GetLightSourceAngle() > 0))
					{
						PenumbraMag = 1.571f; // atan(inf)
					}

					const float PerceptualLum = FMath::Loge(1.f + LightSceneInfoCompact.Color.GetLuminance());

					const FVector LightPosition(LightSceneProxy->GetPosition());
					
					float ViewLightDistance = FVector::Distance(ViewPosition, LightPosition);
					if (LightType == LightType_Directional && GPrioritizeDirectional)
					{
						ViewLightDistance = 0.0f;
					}

#if RANKDEBUG
					LightImportance.Distance = ViewLightDistance;
					LightImportance.PerceptualLum = PerceptualLum;
					LightImportance.PerceptualArea = PerceptualArea;
					LightImportance.PenumbraMag = PenumbraMag;
					LightImportance.LightParameters = LightParameters;
#endif // RANKDEBUG

					LightImportance.Score = 1.f;
					LightImportance.Score /= 1.f + ViewLightDistance * ViewLightDistance; // falloff from light peak
					LightImportance.Score *= PerceptualArea;
					LightImportance.Score *= PerceptualLum;
					LightImportance.Score *= 1.f + (PenumbraMag);

					LightRanking.Add(LightImportance);

				}

			}

			// Sort algorithms for light importance

			struct FCompareImportanceByScore
			{
				FORCEINLINE bool operator()(const FLightImportance& A, const FLightImportance& B) const
				{
					return A.Score > B.Score;
				}
			};

			LightRanking.Sort(FCompareImportanceByScore());

			int32 DenoisedLightCount = 0;
			int32 RTShadowedLightCount = 0;

			// Set highest N ranked lights as desiring RayTracing for the current frame
			for (int32 Index = 0; Index < LightRanking.Num(); Index++)
			{
				const FLightImportance& LightImportance = LightRanking[Index];

				check(LightImportance.LightIndex >= 0);
				if (LightImportance.LightIndex >= 0)
				{
					auto &LightSceneInfoCompact = Scene->Lights[LightImportance.LightIndex];
					check(LightImportance.LightIndex == LightSceneInfoCompact.LightSceneInfo->Id);
					FLightSceneInfo* LightSceneInfo = LightSceneInfoCompact.LightSceneInfo;
					FLightSceneProxy* LightSceneProxy = LightSceneInfo->Proxy;

					if (GMaxRTLights > -1)
					{
						const bool bGetsRTShadow = RTShadowedLightCount < GMaxRTLights || LightImportance.bRequiresDynamicShadow;

						// track promotions and demotions
						if (LightSceneInfo->CastsRaytracedShadowThisFrame() != bGetsRTShadow)
						{
							INC_DWORD_STAT(STAT_ShadowPriorityChanges);
						}

						LightSceneInfo->SetCastsRaytracedShadowThisFrame(bGetsRTShadow);
						LightSceneInfoCompact.bCastRaytracedShadow = bGetsRTShadow;
						RTShadowedLightCount += bGetsRTShadow ? 1 : 0;
					}
					else
					{
						// not limiting lights, but limiting denoiser
						LightSceneInfo->SetCastsRaytracedShadowThisFrame(true);
						LightSceneInfoCompact.bCastRaytracedShadow = 1;
						RTShadowedLightCount++;
					}

					// reset, so we don't read last frame's override in LightRequiresDenosier()
					LightSceneInfo->SetDenoiseShadowThisFrame(true);

					if (LightRequiresDenosier(*LightSceneInfo) && (GMaxDenoisedLights == -1 || DenoisedLightCount < GMaxDenoisedLights))
					{
						DenoisedLightCount++;
					}
					else
					{
						LightSceneInfo->SetDenoiseShadowThisFrame(false);
					}
				}
			}
		}
		...
		{
			// make sure all lights are reset to shadowing for the current frame
			for (TSparseArray<FLightSceneInfoCompact>::TIterator LightIt(Scene->Lights); LightIt; ++LightIt)
			{
				FLightSceneInfoCompact& LightSceneInfoCompact = *LightIt;
				FLightSceneInfo* LightSceneInfo = LightSceneInfoCompact.LightSceneInfo;
...
static void SplitSimpleLightsByShadow(const FSimpleLightArray& SimpleLights, FSimpleLightArray& OutSimpleLights, FSimpleLightArray& OutShadowedSimpleLights)
{
	// only support matched per-view and instance data
	check(SimpleLights.InstancePerViewDataIndices.Num() == 0);
	check(SimpleLights.InstanceData.Num() == SimpleLights.PerViewData.Num());

	for (int32 LightIndex = 0; LightIndex < SimpleLights.InstanceData.Num(); ++LightIndex)
	{
		auto& InstanceData = SimpleLights.InstanceData[LightIndex];
		auto& PerViewData = SimpleLights.PerViewData[LightIndex];

		if (InstanceData.bCastSampledShadows)
		{
			OutShadowedSimpleLights.InstanceData.Add(InstanceData);
			OutShadowedSimpleLights.PerViewData.Add(PerViewData);
		}
		else
		{
			OutSimpleLights.InstanceData.Add(InstanceData);
			OutSimpleLights.PerViewData.Add(PerViewData);
		}
	}
}
...
void FSceneRenderer::GatherAndSortLights(FSortedLightSetSceneInfo& OutSortedLights)
{
	...
	const int32 ParticleLightMode = UseSampledLightingForParticles();
	bool bSampleSimpleLights = ParticleLightMode > 0 && OutSortedLights.SimpleLights.InstancePerViewDataIndices.Num() == 0;

	if (bSampleSimpleLights )
	{
		if (ParticleLightMode == 2)
		{
			//forcing all particle lights to be sampled lights
			FSimpleLightArray NonSampledSimpleLights;

			OutSortedLights.ShadowedSimpleLights = OutSortedLights.SimpleLights;
			OutSortedLights.SimpleLights = NonSampledSimpleLights;
		}
		else {
			const int32 TotalSimpleLightCount = OutSortedLights.SimpleLights.InstanceData.Num();
			FSimpleLightArray NonSampledSimpleLights;

			//alloc the worst case
			NonSampledSimpleLights.InstanceData.Reserve(TotalSimpleLightCount);
			NonSampledSimpleLights.PerViewData.Reserve(TotalSimpleLightCount);
			OutSortedLights.ShadowedSimpleLights.InstanceData.Reserve(TotalSimpleLightCount);
			OutSortedLights.ShadowedSimpleLights.PerViewData.Reserve(TotalSimpleLightCount);

			SplitSimpleLightsByShadow(OutSortedLights.SimpleLights, NonSampledSimpleLights, OutSortedLights.ShadowedSimpleLights);
			OutSortedLights.SimpleLights = NonSampledSimpleLights;
		}
	}
	...
	const bool bRTShadowsEnabled = ShouldRenderRayTracingShadows();
	// ToDo: add ray tracing support checks
	const bool bSampledDirectLighting = ShouldRenderRayTracingSampledLighting();
	const bool bSampleDirectional = SupportSampledLightingForType(LightType_Directional);
	const bool bSamplePoint = SupportSampledLightingForType(LightType_Point);
	const bool bSampleSpot = SupportSampledLightingForType(LightType_Spot);
	const bool bSampleRect = SupportSampledLightingForType(LightType_Rect);
	const bool bSampleLightFunctions = SupportSampledLightingForLightFunctions();

	if (bDynamicShadows && bRTShadowsEnabled)
	{
		PrioritizeLights(Scene, Views);
	}
			...
			&& !ViewFamily.EngineShowFlags.ReflectionOverride)
		{
			...
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
			{
				if (LightSceneInfo->ShouldRenderLight(Views[ViewIndex]))
				{
					...
					SortedLightInfo->SortKey.Fields.bSampledLightingNotSupported = !bSampledDirectLighting
						|| (!LightSceneInfo->Proxy->SupportsSampledLighting())
						|| (!bSampleDirectional && LightSceneInfoCompact.LightType == LightType_Directional)
						|| (!bSamplePoint && LightSceneInfoCompact.LightType == LightType_Point)
						|| (!bSampleSpot && LightSceneInfoCompact.LightType == LightType_Spot)
						|| (!bSampleRect && LightSceneInfoCompact.LightType == LightType_Rect)
						|| (!bSampleLightFunctions && SortedLightInfo->SortKey.Fields.bLightFunction);
					break;
				}
			}
		}
	...
	OutSortedLights.ComplexLightsStart = SortedLights.Num();
	...
	for (int32 LightIndex = 0; LightIndex < SortedLights.Num(); LightIndex++)
	{
		...
		if ((bDrawShadows || bDrawLightFunction || bLightingChannels) && OutSortedLights.AttenuationLightStart == SortedLights.Num())
		{
			check(SortedLightInfo.SortKey.Fields.bTiledDeferredNotSupported);
			OutSortedLights.AttenuationLightStart = LightIndex;
		}

		if ((bDrawShadows || bDrawLightFunction || bLightingChannels) && SortedLightInfo.SortKey.Fields.bSampledLightingNotSupported)
		{
			// complex lights are the last category, can end the loop here
			OutSortedLights.ComplexLightsStart = LightIndex;
			break;
		}
	}
	...
	check(OutSortedLights.ComplexLightsStart >= OutSortedLights.AttenuationLightStart);
}
	...
	const FHairStrandsRenderingData* HairDatas)
{
	...
	const FSimpleLightArray &SampledSimpleLights = SortedLightSet.ShadowedSimpleLights;
...

	{
		...
		if (SortedLightSet.ComplexLightsStart > AttenuationLightStart)
		{
			RDG_EVENT_SCOPE(GraphBuilder, "SampledLights");
			check(ShouldRenderRayTracingSampledLighting());

			TArray<FSortedLightSceneInfo, SceneRenderingAllocator> SampledLights;
			SampledLights.Append(SortedLights.GetData() + AttenuationLightStart, SortedLightSet.ComplexLightsStart - AttenuationLightStart);

			SET_DWORD_STAT(STAT_NumSampledLights, SampledLights.Num() + SampledSimpleLights.InstanceData.Num());

			RenderSampledDirectLighting(GraphBuilder, SceneTexturesUniformBuffer, SampledLights, SampledSimpleLights, HairDatas, SceneColorTexture, LightingChannelsTexture);

			// presently all sampled lights are injected without shadows
			if (GUseTranslucentLightingVolumes && GSupportsVolumeTextureRendering)
			{
				// Inject non-shadowed, non-simple, non-light function lights in to the volume.
				InjectTranslucentVolumeLightingArray(GraphBuilder, SortedLights, AttenuationLightStart, SortedLightSet.ComplexLightsStart);
				...
				if (SimpleLights.InstanceData.Num() > 0)
				{
					...
					SplitSimpleLightsByView(Views, SampledSimpleLights, SimpleLightsByView);
					...
					for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
					{
						...
						if (SimpleLightArray.InstanceData.Num() > 0)
						{
							FViewInfo& View = Views[ViewIndex];
							RDG_GPU_MASK_SCOPE(GraphBuilder, View.GPUMask);
							RDG_EVENT_SCOPE(GraphBuilder, "InjectSampledSimpleLightsTranslucentLighting");
							InjectSimpleTranslucentVolumeLightingArray(GraphBuilder, SimpleLightArray, View, ViewIndex);
						}
					}
				}
		...
		}
		{
			...
			// Draw shadowed and light function lights
			for (int32 LightIndex = SortedLightSet.ComplexLightsStart; LightIndex < SortedLights.Num(); LightIndex++)
			{
				const FSortedLightSceneInfo& SortedLightInfo = SortedLights[LightIndex];
				const FLightSceneInfo& LightSceneInfo = *SortedLightInfo.LightSceneInfo;
				...
				if (bDrawShadows)
				{
					...
					const FLightOcclusionType OcclusionType = GetActiveLightOcclusionType(LightSceneInfo);
					...
					// reduces memory overhead while keeping shadows batched to optimize costs
					{
							...
							(PreprocessedShadowMaskTextures.Num() == 0 || !PreprocessedShadowMaskTextures[LightIndex - AttenuationLightStart]))
						{
							...
							const bool bSharpFallback = CVarFallbackToSharpShadows.GetValueOnRenderThread() != 0;
							...
							for (int32 LightBatchIndex = LightIndex; LightBatchIndex < SortedLights.Num(); LightBatchIndex++)
							{
								...
								const FLightOcclusionType BatchOcclusionType = GetActiveLightOcclusionType(BatchLightSceneInfo);;
								...
								HairResources.bUseHairVoxel = !BatchLightSceneInfo.Proxy->CastsHairStrandsDeepShadow();
								{
									...
									RenderRayTracingShadows(
										...
										&HairResources,
										LightingChannelsTexture,
										bSharpFallback && !bRequiresDenoiser,
