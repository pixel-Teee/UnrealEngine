...
struct FSortedLightSceneInfo
{
	union
	{
		struct
		{
			...
			/** Whether the light doesn't qualify for sampled lighting. */
			uint32 bSampledLightingNotSupported : 1;
...
struct FSortedLightSetSceneInfo
{
	...
	/** First light unsupported by sampled lighting */
	int ComplexLightsStart;

	FSimpleLightArray SimpleLights;
	FSimpleLightArray ShadowedSimpleLights;
...
public:
	...
	/** Whether the light actively casts shadows using ray tracing, may be better placed on a separate data structure*/
	uint32 bCastRayTracedShadowThisFrame : 1;

	/** Whether the light uses a full denoiser pass this frame*/
	uint32 bDenoiseShadowThisFrame : 1;
	...
	/** Controls for whether the light actively casts shadows using ray tracing, may be better placed on a different data structure */
	bool CastsRaytracedShadowThisFrame() const
	{
		return bCastRayTracedShadowThisFrame; 
	}

	void SetCastsRaytracedShadowThisFrame(bool Cast)
	{
		bCastRayTracedShadowThisFrame = Proxy->CastsRaytracedShadow() && Cast;
	}

	bool DenoiseShadowThisFrame() const
	{
		return bDenoiseShadowThisFrame;
	}

	void SetDenoiseShadowThisFrame(bool Cast)
	{
		bDenoiseShadowThisFrame = Proxy->CastsRaytracedShadow() && Cast;
	}
