...
static TAutoConsoleVariable<int32> CVarRayTracingTranslucencyMask(
	TEXT("r.RayTracing.Translucency.Mask"),
	1,
	TEXT(" 0: trace translucency rays for all pixels \n")
	TEXT(" 1: rasterize mask of translucent RT geometry and only trace for covered pixels"),
	ECVF_RenderThreadSafe);
...
static TAutoConsoleVariable<int32> CVarRayTracingVisualizeBVHShowSM(
	TEXT("r.RayTracing.VisualizeBVH.ShowFlags.StaticMesh"),
	1,
	TEXT("Include standard static mesh bounds when visualizing BVH"));
...
static TAutoConsoleVariable<int32> CVarRayTracingPreGather(
	TEXT("r.RayTracing.PreGather"),
	1,
	TEXT("Whether to gather relevant ray tracing primitives during InitViews.\n"),
	ECVF_RenderThreadSafe
);
...
bool FDeferredShadingSceneRenderer::GatherRayTracingWorldInstances(FRHICommandListImmediate& RHICmdList)
{
	...
	FRHIRayTracingGeometry* UnitCube = Scene->UnitCubeRayTracingGeometry.RayTracingGeometry.RayTracingGeometryRHI;
	const bool bGatherAABB = (UnitCube != nullptr) && (ViewFamily.EngineShowFlags.VisualizeBVHComplexity || ViewFamily.EngineShowFlags.VisualizeBVHOverlap);
...

	{
		...
		if (CVarRayTracingPreGather.GetValueOnRenderThread() == 0)
		{
			int32 BroadIndex = 0;
		...
		else
		{
			...
			for (TConstSetBitIterator< FDefaultBitArrayAllocator> BitIt(Scene->RayTracingRelevantPrimitiveMap); BitIt; ++BitIt)
			{
				...
				for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
				{
					...
					if (Scene->RayTracingStaticRelevantPrimitiveMap[PrimitiveIndex] && View.Family->EngineShowFlags.StaticMeshes)
					{
						Item.ViewIndex = ViewIndex;
						RelevantPrimitives.Add(Item);
					}
		...
		for (const FRelevantPrimitive& RelevantPrimitive : RelevantPrimitives)
		{
			...
			if (RayTracingInstances.Num() > 0)
			{
				for (FRayTracingInstance& Instance : RayTracingInstances)
				{
					...
					if (bGatherAABB && Instance.InstanceAABBs.Num() > 0)
					{
						FRayTracingGeometryInstance AABBInstance;

						check(UnitCube);
						AABBInstance.GeometryRHI = UnitCube;
						AABBInstance.bDoubleSided = false;
						AABBInstance.bForceOpaque = false;
						AABBInstance.Mask = 0xff;
						AABBInstance.UserData.Add(0);
						AABBInstance.NumTransforms = RayTracingInstance.NumTransforms;

						for (FViewInfo& View : Views)
						{
							for (FBox& InstanceAABB : Instance.InstanceAABBs)
							{
								FVector ScaleFactor = InstanceAABB.GetSize();
								...
								AABBInstance.Transforms.Add(Transform.ConcatTranslation(InstanceAABB.Min));
							}

							View.RayTracingAABBInstances.Add(AABBInstance);
						}
					}
...

	{
		...
		for (const FRelevantPrimitive& RelevantPrimitive : RelevantPrimitives)
		{
			...
			if (bGatherAABB && CVarRayTracingVisualizeBVHShowSM->GetBool())
			{
				FRayTracingGeometryInstance AABBInstance;
				...
				FPrimitiveSceneProxy* SceneProxy = Scene->PrimitiveSceneProxies[PrimitiveIndex];
				FBox AABB = SceneProxy->GetBounds().GetBox();
				FMatrix Transform = FMatrix::Identity;
				FVector ScaleFactor = AABB.GetSize();
				Transform.M[0][0] *= ScaleFactor.X;
				Transform.M[1][1] *= ScaleFactor.Y;
				Transform.M[2][2] *= ScaleFactor.Z;
				AABBInstance.Transforms.Add(Transform.ConcatTranslation(AABB.Min));
...
bool FDeferredShadingSceneRenderer::DispatchRayTracingWorldUpdates(FRHICommandListImmediate& RHICmdList)
{
	...
	const bool bGatherAABB = ViewFamily.EngineShowFlags.VisualizeBVHComplexity || ViewFamily.EngineShowFlags.VisualizeBVHOverlap;

	FViewInfo& ReferenceView = Views[0];

	GatherLightFunctionLights(Scene, ReferenceView.GetFeatureLevel(), CanUseRayTracingLightingMissShader(ReferenceView.GetShaderPlatform()));

	check(Scene->RayTracingLightFunctionLights.Num() <= RAY_TRACING_MAX_LIGHT_FUNCTIONS);
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		SceneInitializer.NumMissShaderSlots = RAY_TRACING_NUM_MISS_SHADER_SLOTS + RAY_TRACING_MAX_LIGHT_FUNCTIONS;
		SceneInitializer.NumCallableShaderSlots = RAY_TRACING_MAX_LIGHT_FUNCTIONS;
		...
		PrepareRayTracingReflectionCaptures(View, *Scene, RayGenShaders);
		...
		View.RayTracingLightData = CreateRayTracingLightData(RHICmdList, Scene, View,
		...
		if (RayGenShaders.Num())
		{
			...
			View.RayTracingMaterialPipeline = BindRayTracingMaterialPipeline(RHICmdList, View,
				RayGenShaders,
				DefaultHitShader,
				Scene->RayTracingLightFunctionLights
			);
		}
		...
		BindLightFunctionShaders(RHICmdList, Scene, Scene->RayTracingLightFunctionLights, View, CanUseRayTracingLightingMissShader(View.GetShaderPlatform()));

		//EHartNV TODO: make this work with the async logic?
		if (bGatherAABB)
		{
			FRayTracingSceneInitializer AABBInitializer;
			AABBInitializer.Instances = View.RayTracingAABBInstances;
			AABBInitializer.ShaderSlotsPerGeometrySegment = 1;

			View.RayTracingSceneBVH.RayTracingSceneRHI = RHICreateRayTracingScene(AABBInitializer);
			RHICmdList.BuildAccelerationStructure(View.RayTracingSceneBVH.RayTracingSceneRHI);
		}
	}
	...
	AddPass(GraphBuilder, [this](FRHICommandListImmediate& RHICmdList)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			...
			if (View.RayTracingMaterialBindings.Num())
			{
				...
				PrepareRayTracingReflectionCaptures(View, *Scene, DeferredMaterialRayGenShaders);
...
void FDeferredShadingSceneRenderer::Render(FRHICommandListImmediate& RHICmdList)
{
	...
	if (bRenderDeferredLighting)
	{
		...
		if (GbEnableAsyncComputeTranslucencyLightingVolumeClear && GSupportsEfficientAsyncCompute && GEnableAsyncCompute)
		{
	...
	if (VisualizeRayTracingTiming(Views[0]))
	{
		SceneContext.AllocateRayTracingTimingTarget(RHICmdList);
		RHICmdList.ClearUAVUint(SceneContext.RayTracingTiming->GetRenderTargetItem().UAV, FUintVector4(0));
	}
	...
	if (bRenderDeferredLighting)
	{
		...
		if (bRayTracingEnabled)
		{
			// This mask is also overloaded for shadow and AO bias when WPO is not fully evaluated
		...
		if ((GbEnableAsyncComputeTranslucencyLightingVolumeClear && GSupportsEfficientAsyncCompute && GEnableAsyncCompute) == false)
		{
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
			{
				const FViewInfo& View = Views[ViewIndex];
	...
	if (bCanOverlayRayTracingOutput && TranslucencyViewsToRender != ETranslucencyView::None)
	{
		...
		bool bAnyViewHasHybridTranslucency = false;
		bool bAnyViewNeedsHybridRasterPass = false;
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			...
			bAnyViewHasHybridTranslucency = bAnyViewHasHybridTranslucency || HybridTranslucencyMode != EHybridTranslucencyMode::None;
			bAnyViewNeedsHybridRasterPass = bAnyViewNeedsHybridRasterPass || HybridTranslucencyMode == EHybridTranslucencyMode::Mode2;
		}
#endif

		SceneContext.AllocateHybridTranslucencyTargets(GraphBuilder.RHICmdList, bAnyViewHasHybridTranslucency);

		// setup the stencil buffer with regions touched by translucency
		if (CVarRayTracingTranslucencyMask.GetValueOnRenderThread() != 0)
		{
			SCOPED_DRAW_EVENT(RHICmdList, TranslucencyRayTracingMask);

			ETranslucencyView TranslucencyMaskViewsToRender = TranslucencyViewsToRender;
			EnumAddFlags(TranslucencyMaskViewsToRender, ETranslucencyView::RayTracingMask);

			RenderTranslucency(GraphBuilder, SceneColorTexture, SceneDepthTexture, HairDatas, &SeparateTranslucencyTextures, TranslucencyMaskViewsToRender);
		}

		// Raytracing doesn't need the distortion effect.
		const bool bShouldRenderDistortion = TranslucencyViewsToRender != ETranslucencyView::RayTracing || bAnyViewHasHybridTranslucency;
		...
		if (bAnyViewNeedsHybridRasterPass)
		{
			EnumAddFlags(TranslucencyViewsToRender, ETranslucencyView::RayTracingRaster);
		}
#endif
		...
		if (bShouldRenderDistortion)
		{
			...
			RenderDistortion(GraphBuilder, SceneColorTexture.Target, SceneDepthTexture.Target, bAnyViewNeedsHybridRasterPass);
			AddServiceLocalQueuePass(GraphBuilder);
		}
	...
	if (bRayTracingEnabled)
	{
		...
		if (VisualizeRayTracingTiming(Views[0]))
		{
			RenderVisualizeRayTracingTiming(GraphBuilder, SceneColorTexture.Target);
		}

		if (ViewFamily.EngineShowFlags.VisualizeBVHComplexity || ViewFamily.EngineShowFlags.VisualizeBVHOverlap)
		{
			RenderRayTracingVisualizeBVH(GraphBuilder, SceneTextures, SceneColorTexture.Target);
		}
	}
#endif
