...
// Must match struct definition of FPackedSampledLightingData in "DirectLightSamplingCommon.ush
struct FRayTracingSampledLightingData
{
	int32 Type;
	int32 LightProfileIndex;
	int32 RectLightTextureIndex;
	uint32 FlagsLightFunctionAndMask;
...
// Structure for sampled direct lighting
struct FSampledLightingHistory
{
	...
	// Array of processed light components, used to track history
	TArray<FRayTracingSampledLightingData> LightData;
...
// Plugins can derive from this and use it for their own purposes
class RENDERER_API ICustomDenoisePolychromaticPenumbraHarmonicsHistory : public IRefCountedObject
{
public:
	virtual ~ICustomDenoisePolychromaticPenumbraHarmonicsHistory() {}
};
...
struct FPreviousViewInfo
{
...
#if RHI_RAYTRACING
	// History for ray traced translucency
	FTemporalAAHistory RayTracedTranslucencyHistory;
#endif
	...
	// Custom denoiser history, used by plugins
	TRefCountPtr<ICustomDenoisePolychromaticPenumbraHarmonicsHistory> CustomDenoisePolychromaticPenumbraHarmonicsHistory;
...
public:
	...
	// instances for the BVH only tree
	TArray<FRayTracingGeometryInstance, SceneRenderingAllocator> RayTracingAABBInstances;

	// BVH only raytracing scene specific to this view
	FRayTracingScene RayTracingSceneBVH;
