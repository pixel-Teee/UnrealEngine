...
void FReflectionEnvironmentCubemapArray::InitDynamicRHI()
{
	if (SupportsTextureCubeArray(GetFeatureLevel()))
	{
		...
		{
			const ETextureCreateFlags Flags = TexCreate_ShaderResource | TexCreate_RenderTargetable | TexCreate_UAV;
			FPooledRenderTargetDesc RayTracedDesc = FPooledRenderTargetDesc::CreateCubemapArrayDesc(
				CubemapSize,
				// Alpha stores sky mask
				PF_FloatRGBA,
				FClearValueBinding::None,
			...
			// Allocate TextureCubeArray for the scene's ray traced reflection captures
			GRenderTargetPool.FindFreeElement(RHICmdList, RayTracedDesc, RayTraceReflectionEnvs, TEXT("RayTracedReflectionEnvs"));
		}
	}
}
...
void FReflectionEnvironmentCubemapArray::ReleaseCubeArray()
{
	...
	GRenderTargetPool.FreeUnusedResource(RayTraceReflectionEnvs);
}
