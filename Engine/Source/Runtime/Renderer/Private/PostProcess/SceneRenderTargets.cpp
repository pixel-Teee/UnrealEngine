...
FSceneRenderTargets::FSceneRenderTargets(const FViewInfo& View, const FSceneRenderTargets& SnapshotSource)
	...
	, TranslucencyLayersColor(GRenderTargetPool.MakeSnapshot(SnapshotSource.TranslucencyLayersColor))
	, TranslucencyLayersDepth(GRenderTargetPool.MakeSnapshot(SnapshotSource.TranslucencyLayersDepth))
	, RayTracingTiming(GRenderTargetPool.MakeSnapshot(SnapshotSource.RayTracingTiming))
...
void FSceneRenderTargets::AllocateRayTracingTimingTarget(FRHICommandList& RHICmdList)
{
	if (!RayTracingTiming)
	{
		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(BufferSize, PF_R32_UINT, FClearValueBinding::Black, TexCreate_None, TexCreate_RenderTargetable | TexCreate_ShaderResource | TexCreate_UAV, false));

		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, RayTracingTiming, TEXT("RayTracingTiming"));
	}
}
...
void FSceneRenderTargets::AllocateHybridTranslucencyTargets(FRHICommandListImmediate& RHICmdList, bool bAnyViewUsesHybridTranslucency)
{
	// Create the translucency compositing targets
	if (CurrentFeatureLevel >= ERHIFeatureLevel::SM5)
	{
		// default to trivial sizes to have a stub to keep the render graph happy without eating memory
		FIntPoint TranslucencyBufferSize = FIntPoint(1, 1);
		int32 ArraySize = 1;

		if(bAnyViewUsesHybridTranslucency)
		{
			TranslucencyBufferSize = BufferSize;
			ArraySize = MaxHybridTranslucencyLayers();
		}

		FPooledRenderTargetDesc Desc(FPooledRenderTargetDesc::Create2DDesc(TranslucencyBufferSize, PF_FloatRGBA, FClearValueBinding::Transparent, TexCreate_None, TexCreate_UAV | TexCreate_RenderTargetable, false));
		Desc.ArraySize = ArraySize;
		Desc.bIsArray = true;

		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, TranslucencyLayersColor, TEXT("TranslucencyLayersColor"));

		Desc.Format = PF_R32_FLOAT;
		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, TranslucencyLayersDepth, TEXT("TranslucencyLayersDepth"));
	}
}
...
void FSceneRenderTargets::ReleaseAllTargets()
{
	...
	TranslucencyLayersColor.SafeRelease();
	TranslucencyLayersDepth.SafeRelease();

	RayTracingTiming.SafeRelease();
