...
static void ComputeAndMarkRayTracingRelevanceForView(const FScene* Scene, FViewInfo& View)
{
	...
	static const auto RayTracingCullingCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.Culling"));
	static const auto RayTracingCullingRadiusCVar = IConsoleManager::Get().FindTConsoleVariableDataFloat(TEXT("r.RayTracing.Culling.Radius"));
	static const auto RayTracingCullingAngleCVar = IConsoleManager::Get().FindTConsoleVariableDataFloat(TEXT("r.RayTracing.Culling.Angle"));
	static const auto RayTracingSceneCapturesCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.RayTracing.SceneCaptures"));

	const bool bApplyCulling = RayTracingCullingCVar->GetValueOnRenderThread() > 0;
	const float CullingRadius = RayTracingCullingRadiusCVar->GetValueOnRenderThread();
	const float CullAngleThreshold = RayTracingCullingAngleCVar->GetValueOnRenderThread();
	const float AngleThresholdRatio = FMath::Tan(CullAngleThreshold * PI / 180.0f);

	const int32 RayTracingSceneCaptures = RayTracingSceneCapturesCVar->GetInt();
	...
	ParallelFor(NumTasks,
		[Scene, &View, bApplyCulling, CullingRadius, AngleThresholdRatio, RayTracingSceneCaptures](int32 TaskIndex)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(ScanRayTracingInstances_Loop);
		const int32 BitArrayNumInner = Scene->PrimitiveSceneProxies.Num();
		...
		for (int32 WordIndex = TaskWordOffset; WordIndex < TaskWordOffset + FrustumCullNumWordsPerTask && WordIndex * NumBitsPerDWORD < BitArrayNumInner; WordIndex++)
		{
			...
			for (int32 BitSubIndex = 0; BitSubIndex < NumBitsPerDWORD && WordIndex * NumBitsPerDWORD + BitSubIndex < BitArrayNumInner; BitSubIndex++, Mask <<= 1)
			{
				...
				bool bShouldRayTraceSceneCapture = RayTracingSceneCaptures > 0 || (RayTracingSceneCaptures == -1 && View.bSceneCaptureUsesRayTracing);
...
#if RHI_RAYTRACING
				if (PrimitiveSceneInfo->bIsRayTracingRelevant)
				{
					TranslucentPrimCount.Add(ETranslucencyPass::TPT_RayTracingTranslucencyMask, ViewRelevance.bUsesSceneColorCopy);
				}

				if (!PrimitiveSceneInfo->bIsRayTracingRelevant
					|| (!PrimitiveSceneInfo->bIsVisibleInRayTracing && PrimitiveSceneInfo->bIsRayTracingRelevant)
					)
				{
					if (View.Family->AllowTranslucencyAfterDOF())
					{
						if (ViewRelevance.bNormalTranslucency)
						{
							TranslucentPrimCount.Add(ETranslucencyPass::TPT_StandardTranslucencyRTRaster, ViewRelevance.bUsesSceneColorCopy);
						}

						if (ViewRelevance.bSeparateTranslucency)
						{
							TranslucentPrimCount.Add(ETranslucencyPass::TPT_TranslucencyAfterDOFRTRaster, ViewRelevance.bUsesSceneColorCopy);
						}

						if (ViewRelevance.bSeparateTranslucencyModulate)
						{
							TranslucentPrimCount.Add(ETranslucencyPass::TPT_TranslucencyAfterDOFModulateRTRaster, ViewRelevance.bUsesSceneColorCopy);
						}
					}
					else // Otherwise, everything is rendered in a single bucket. This is not related to whether DOF is currently enabled or not.
					{
						// When using all translucency, Standard and AfterDOF are sorted together instead of being rendered like 2 buckets.
						TranslucentPrimCount.Add(ETranslucencyPass::TPT_AllTranslucencyRTRaster, ViewRelevance.bUsesSceneColorCopy);
					}
				}
#endif
		...
		for (int32 StaticPrimIndex = 0, Num = RelevantStaticPrimitives.NumPrims; StaticPrimIndex < Num; ++StaticPrimIndex)
		{
			...
			for(int32 MeshIndex = 0;MeshIndex < NumStaticMeshes;MeshIndex++)
			{
				...
				if (LODToRender.ContainsLOD(StaticMeshRelevance.LODIndex))
				{
					...
					bool bRayTracingShadowBias = false;

#if RHI_RAYTRACING
					if (IsRayTracingEnabled())
					{
						bRayTracingShadowBias = PrimitiveSceneInfo->Proxy->UseRayTracingShadowBias();
					}
#endif

					if (StaticMeshRelevance.bUsesWorldPositionOffset && bRayTracingShadowBias)
					{
						// Actual evaluation is skipped for RT, so let's go down this same path as dithered LOD so we offset the shadow geometry
					...
					if (ViewRelevance.bDrawRelevance)
					{
							...
							&& ViewRelevance.bRenderInMainPass)
						{
...
#if RHI_RAYTRACING
							// Add to special translucent RT mask list if primitive is RT relevant and RT is enabled
							// EHartNV ToDo - should this only add when TY or RT translucency is enabled? (determining RT translucency can be hard due to per-view settings)
							if (StaticMesh.PrimitiveSceneInfo->bIsRayTracingRelevant)
							{
								DrawCommandPacket.AddCommandsForMesh(PrimitiveIndex, PrimitiveSceneInfo, StaticMeshRelevance, StaticMesh, Scene, bCanCache, EMeshPass::TranslucencyRTMask);
							}

							if (!StaticMesh.PrimitiveSceneInfo->bIsRayTracingRelevant
								|| (!StaticMesh.PrimitiveSceneInfo->bIsVisibleInRayTracing && StaticMesh.PrimitiveSceneInfo->bIsRayTracingRelevant)
								)
							{
								if (View.Family->AllowTranslucencyAfterDOF())
								{
									if (ViewRelevance.bNormalTranslucency)
									{
										DrawCommandPacket.AddCommandsForMesh(PrimitiveIndex, PrimitiveSceneInfo, StaticMeshRelevance, StaticMesh, Scene, bCanCache, EMeshPass::TranslucencyStandardRTRaster);
									}

									if (ViewRelevance.bSeparateTranslucency)
									{
										auto MeshPass = EMeshPass::TranslucencyAfterDOFRTRaster;
										DrawCommandPacket.AddCommandsForMesh(PrimitiveIndex, PrimitiveSceneInfo, StaticMeshRelevance, StaticMesh, Scene, bCanCache, MeshPass);
									}

									if (ViewRelevance.bSeparateTranslucencyModulate)
									{
										auto MeshPass = EMeshPass::TranslucencyAfterDOFModulateRTRaster;
										DrawCommandPacket.AddCommandsForMesh(PrimitiveIndex, PrimitiveSceneInfo, StaticMeshRelevance, StaticMesh, Scene, bCanCache, MeshPass);
									}
								}
								else
								{
									// Otherwise, everything is rendered in a single bucket. This is not related to whether DOF is currently enabled or not.
									// When using all translucency, Standard and AfterDOF are sorted together instead of being rendered like 2 buckets.
									DrawCommandPacket.AddCommandsForMesh(PrimitiveIndex, PrimitiveSceneInfo, StaticMeshRelevance, StaticMesh, Scene, bCanCache, EMeshPass::TranslucencyAllRTRaster);
								}
							}
#endif
							...
							if (ViewRelevance.bDistortion)
							{
								...
								{
									DrawCommandPacket.AddCommandsForMesh(PrimitiveIndex, PrimitiveSceneInfo, StaticMeshRelevance, StaticMesh, Scene, bCanCache, EMeshPass::DistortionRaster);
								}
#endif
...
void ComputeDynamicMeshRelevance(EShadingPath ShadingPath, bool bAddLightmapDensityCommands, const FPrimitiveViewRelevance& ViewRelevance, const FMeshBatchAndRelevance& MeshBatch, FViewInfo& View, FMeshPassMask& PassMask, FPrimitiveSceneInfo* PrimitiveSceneInfo, const FPrimitiveBounds& Bounds)
{
	...
	if (ViewRelevance.bDrawRelevance && (ViewRelevance.bRenderInMainPass || ViewRelevance.bRenderCustomDepth || ViewRelevance.bRenderInDepthPass))
	{
		...
		if (ViewRelevance.bRenderInMainPass || ViewRelevance.bRenderCustomDepth)
		{
			...
			if (ViewRelevance.bUsesWorldPositionOffset && bRayTracingShadowBias)
			{
				// NV: We're overloading this path to include any which uses inexact geometry -> potentially skipping WPO eval
				PassMask.Set(EMeshPass::DitheredLODFadingOutMaskPass);
				View.NumVisibleDynamicMeshElements[EMeshPass::DitheredLODFadingOutMaskPass] += NumElements;
			}
		}
	}
		...
		&& ViewRelevance.bRenderInMainPass)
	{
...
#if RHI_RAYTRACING
		if (MeshBatch.PrimitiveSceneProxy->IsRayTracingRelevant())
		{
			PassMask.Set(EMeshPass::TranslucencyRTMask);
			View.NumVisibleDynamicMeshElements[EMeshPass::TranslucencyRTMask] += NumElements;
		}

		if (!MeshBatch.PrimitiveSceneProxy->IsRayTracingRelevant()
			|| (!MeshBatch.PrimitiveSceneProxy->IsVisibleInRayTracing() && MeshBatch.PrimitiveSceneProxy->IsRayTracingRelevant())
#if WITH_EDITOR
			// Force to render the primitives in raster to fix the issue that translucent materials are all gone in multiple views 
			// when ray traced translucency is enabled.
			|| (!View.bIsGameView && !PrimitiveSceneInfo->CachedRayTracingMeshCommandIndicesPerLOD.IsValidIndex(0) && PrimitiveSceneInfo->bIsRayTracingRelevant)
#endif
			)
		{
			if (View.Family->AllowTranslucencyAfterDOF())
			{
				if (ViewRelevance.bNormalTranslucency)
				{
					PassMask.Set(EMeshPass::TranslucencyStandardRTRaster);
					View.NumVisibleDynamicMeshElements[EMeshPass::TranslucencyStandardRTRaster] += NumElements;
				}

				if (ViewRelevance.bSeparateTranslucency)
				{
					auto MeshPass = EMeshPass::TranslucencyAfterDOFRTRaster;
					PassMask.Set(MeshPass);
					View.NumVisibleDynamicMeshElements[MeshPass] += NumElements;
				}
			...
			{
				PassMask.Set(EMeshPass::TranslucencyAllRTRaster);
				View.NumVisibleDynamicMeshElements[EMeshPass::TranslucencyAllRTRaster] += NumElements;
			}
		}
#endif
		...
		if (ViewRelevance.bDistortion)
		{
			...
			{
				PassMask.Set(EMeshPass::DistortionRaster);
				View.NumVisibleDynamicMeshElements[EMeshPass::DistortionRaster] += NumElements;
			}
#endif
...
#if RHI_RAYTRACING
	// Record whether this frame needs any ray tracing data
	if (IsRayTracingEnabled() && Views.Num() > 0)
	{
		bAnyRayTracingPassEnabled = true;

		bool bLocalAnyRayTracingPassEnabled = false;
		bool bPathTracingOrDebugViewEnabled = false;
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			bLocalAnyRayTracingPassEnabled |= AnyRayTracingPassEnabled(Scene, Views[ViewIndex]);
			bPathTracingOrDebugViewEnabled |= !CanOverlayRayTracingOutput(Views[ViewIndex]);
		}

		bAnyRayTracingPassEnabled &= bLocalAnyRayTracingPassEnabled;

		// when ray tracing is forced off, debug can enable it
		bAnyRayTracingPassEnabled &= !(!bAnyRayTracingPassEnabled && !bPathTracingOrDebugViewEnabled);
	}

	static const auto RayTracingPreGatherCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.PreGather"));
	const bool bPreGather = RayTracingPreGatherCVar->GetValueOnRenderThread() != 0;
	if (bAnyRayTracingPassEnabled && bPreGather)
	{
...
bool FDeferredShadingSceneRenderer::InitViews(FRHICommandListImmediate& RHICmdList, FExclusiveDepthStencil::Type BasePassDepthStencilAccess, struct FILCUpdatePrimTaskData& ILCTaskData)
{
...

	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			View.IESLightProfile2DResource = View.ViewState ? &View.ViewState->IESLightProfile2DResources : nullptr;
#endif
