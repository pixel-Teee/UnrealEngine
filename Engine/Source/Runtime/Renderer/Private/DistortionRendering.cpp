...
TRDGUniformBufferRef<FDistortionPassUniformParameters> CreateDistortionPassUniformBuffer(FRDGBuilder& GraphBuilder, const FViewInfo& View, bool bNeedHybridTranslucencyDistortion=false)
{
...
#if RHI_RAYTRACING
	if (ShouldRenderRayTracingTranslucency(View) && bNeedHybridTranslucencyDistortion)
	{
		FSceneRenderTargets& SceneContext = FSceneRenderTargets::Get(GraphBuilder.RHICmdList);

		static const auto PrimaryRaysHalfResCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RayTracing.Translucency.HalfRes"));
		...
		Parameters->RayTracingTranslucencyType = 2;
		Parameters->RayTracingTranslucencyLayerCount = MaxTranslucencyLayers;
		Parameters->RayTracingTranslucencyUpscale = PrimaryRaysHalfResCVar ? PrimaryRaysHalfResCVar->GetValueOnRenderThread() : -1;
		Parameters->RayTracingTranslucencyLayerDepthThreshold = HybridTranslucencyDepthBiasCVar ? HybridTranslucencyDepthBiasCVar->GetValueOnAnyThread() : 0.2f;// Working as DepthBias in HT mode 2
		Parameters->RayTracingTranslucencyLayersColor = SceneContext.TranslucencyLayersColor->GetRenderTargetItem().ShaderResourceTexture;
		Parameters->RayTracingTranslucencyLayersDepth = SceneContext.TranslucencyLayersDepth->GetRenderTargetItem().ShaderResourceTexture;
	}
	else
	{
		Parameters->RayTracingTranslucencyType = 0;
		Parameters->RayTracingTranslucencyLayerCount = 0;
		Parameters->RayTracingTranslucencyUpscale = -1;
		Parameters->RayTracingTranslucencyLayerDepthThreshold = -1.0f;
		Parameters->RayTracingTranslucencyLayersColor = GSystemTextures.TextureArrayBlackDummy->GetRenderTargetItem().ShaderResourceTexture;
		Parameters->RayTracingTranslucencyLayersDepth = GSystemTextures.TextureArrayBlackDummy->GetRenderTargetItem().ShaderResourceTexture;
	}
#endif
...
bool FDeferredShadingSceneRenderer::ShouldRenderDistortion(bool bNeedHybridTranslucencyDistortion) const
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		bool bHasAnyDraw = bNeedHybridTranslucencyDistortion ? View.ParallelMeshDrawCommandPasses[EMeshPass::DistortionRaster].HasAnyDraw() : View.ParallelMeshDrawCommandPasses[EMeshPass::Distortion].HasAnyDraw();
		if (View.bHasDistortionPrimitives && View.ShouldRenderView() && bHasAnyDraw)
		{
			return true;
		}
	}
...
void FDeferredShadingSceneRenderer::RenderDistortion(FRDGBuilder& GraphBuilder, FRDGTextureRef SceneColorTexture, FRDGTextureRef SceneDepthTexture, bool bNeedHybridTranslucencyDistortion)
{
	check(SceneDepthTexture);
	check(SceneColorTexture);

	if (!ShouldRenderDistortion(bNeedHybridTranslucencyDistortion))
	{
		return;
	}
...

	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			PassParameters->Pass = CreateDistortionPassUniformBuffer(GraphBuilder, View, bNeedHybridTranslucencyDistortion);
			...
			GraphBuilder.AddPass(
				...
				[this, &View, bNeedHybridTranslucencyDistortion](FRHICommandListImmediate& RHICmdList)
			{
				QUICK_SCOPE_CYCLE_COUNTER(STAT_FSceneRender_RenderDistortion_Accumulate_Meshes);
				Scene->UniformBuffers.UpdateViewUniformBuffer(View);

				SetStereoViewport(RHICmdList, View);
				EMeshPass::Type MeshPassType = bNeedHybridTranslucencyDistortion ? EMeshPass::DistortionRaster : EMeshPass::Distortion;
...
FMeshPassProcessor* CreateDistortionRasterPassProcessor(const FScene* Scene, const FSceneView* InViewIfDynamicMeshCommand, FMeshPassDrawListContext* InDrawListContext)
{
	FMeshPassProcessorRenderState DistortionPassState;
	DistortionPassState.SetViewUniformBuffer(Scene->UniformBuffers.ViewUniformBuffer);
	DistortionPassState.SetInstancedViewUniformBuffer(Scene->UniformBuffers.InstancedViewUniformBuffer);
...
FRegisterPassProcessorCreateFunction RegisterDistortionRasterPass(&CreateDistortionRasterPassProcessor, EShadingPath::Deferred, EMeshPass::DistortionRaster, EMeshPassFlags::MainView);