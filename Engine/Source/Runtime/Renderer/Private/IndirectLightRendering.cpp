...
TAutoConsoleVariable<int32> CVarRayTracingReflectionsRayTraceEnvironmentCaptures(
	TEXT("r.RayTracing.Reflections.RayTraceEnvironmentCaptures"), 0,
	TEXT("Real time ray traced environment captures (default = 0)"),
	ECVF_RenderThreadSafe);
...
void SetupReflectionUniformParameters(const FViewInfo& View, FReflectionUniformParameters& OutParameters)
{
	...
	{
#if RHI_RAYTRACING
		bool bRayTraceEnvCaptures = CVarRayTracingReflectionsRayTraceEnvironmentCaptures.GetValueOnRenderThread() == 1
			&& Scene->ReflectionSceneData.CubemapArray.IsValidForRayTracing();
		if (bRayTraceEnvCaptures)
		{
			CubeArrayTexture = Scene->ReflectionSceneData.CubemapArray.GetRayTraceRenderTarget().ShaderResourceTexture;
		}
		else if(Scene->ReflectionSceneData.CubemapArray.IsValid())
		{
	...
	FHairStrandsRenderingData* HairDatas)
{
	...
	for (FViewInfo& View : Views)
	{
		...
		{
			RenderRayTracingReflectionCaptures(
				GraphBuilder,
				SceneTextures,
				View,
