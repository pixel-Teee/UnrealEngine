...
void FDeferredShadingSceneRenderer::RenderRayTracingDebug(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef SceneColorTexture)
{
	...
	if (DebugVisualizationMode == RAY_TRACING_DEBUG_VIZ_PRIMARY_RAYS) 
	{
		...
		ERayTracingPrimaryRaysFlag Flags = ERayTracingPrimaryRaysFlag::ConsiderSurfaceScatter;

		if (VisualizeRayTracingTiming(View))
		{
			Flags |= ERayTracingPrimaryRaysFlag::TimeTracing;
		}

		RenderRayTracingPrimaryRaysView(
				GraphBuilder, View, &OutputColor, &HitDistanceTexture, 1, 1,
	...
	if (VisualizeRayTracingTiming(View))
	{
		RayGenParameters->AccumulateTime = 1;
		RayGenParameters->CumulativeTime = GraphBuilder.CreateUAV(GraphBuilder.RegisterExternalTexture(SceneContext.RayTracingTiming));
	}
	else
	{
		RayGenParameters->AccumulateTime = 0;
		// bogus UAV to just keep validation happy as it is dynamically unused
		RayGenParameters->CumulativeTime = GraphBuilder.CreateUAV(GraphBuilder.RegisterExternalTexture(SceneContext.GetSceneColor()));
	}
