...
static TAutoConsoleVariable<float> CVarRayTracingReflectionsRoughnessMultiplier(
	TEXT("r.RayTracing.Reflections.RoughnessMultiplier"),
	1.0f,
	TEXT("Multiplies reflected RT roughness, can be used to reduce the sampling cone (min=0, max=1, default=1)"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarRayTracingReflectionsTiming(
	TEXT("r.RayTracing.Reflections.Timing"),
	3,
	TEXT("Time cost of ray traced reflections ( 0 - off, 1 - shaded rays. 2 - material gather, 3 - all"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarRayTracingReflectionsRayTraceEnvironmentCapturesCubeUpdateLimit(
	TEXT("r.RayTracing.Reflections.RayTraceEnvironmentCaptures.CubeUpdateLimit"), 
	0,
	TEXT("Limits number of RT cubemap captures updated per frame. 0 is unlimited. (default = 0)"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarRayTracingReflectionsRayTraceEnvironmentCapturesTexelUpdateFrequency(
	TEXT("r.RayTracing.Reflections.RayTraceEnvironmentCaptures.TexelUpdateFrequency"), 
	0,
	TEXT("Limits update frequency for better performance. Texels will be updated round-robin in a grid pattern.\n")
	TEXT("0: All texels update every frame (Default)\n")
	TEXT("1: Every 4 frames\n")
	TEXT("2: Every 16 frames\n")
	TEXT("3: Every 64 frames\n"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarRayTracingReflectionsRayTraceEnvironmentCapturesDisableSkyMask(
	TEXT("r.RayTracing.Reflections.RayTraceEnvironmentCaptures.DisableSkyMask"),
	0,
	TEXT("When enabled, will not mask pixels classified as part of the sky in reflection captures. (default = 0)"),
	ECVF_RenderThreadSafe);

extern TAutoConsoleVariable<int32> CVarRayTracingReflectionsRayTraceEnvironmentCaptures;

DECLARE_GPU_STAT_NAMED(RayTracingReflectionCaptures, TEXT("Ray Tracing Reflection Captures"));
...
static int32 GetMaxReflectionBounces(const FViewInfo& View)
{
	return GRayTracingReflectionsMaxBounces > -1 ? GRayTracingReflectionsMaxBounces : View.FinalPostProcessSettings.RayTracingReflectionsMaxBounces;
}
...
extern int32 GetReflectionsSPP(const FViewInfo& View);
...
class FDownsampleCubeFaceArrayCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FDownsampleCubeFaceArrayCS);
	SHADER_USE_PARAMETER_STRUCT(FDownsampleCubeFaceArrayCS, FGlobalShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FIntPoint, ArrayIndexRange)
		SHADER_PARAMETER_TEXTURE(TextureCubeArray, SourceCubemapArrayTexture)
		SHADER_PARAMETER_SAMPLER(SamplerState, SourceCubemapSampler)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2DArray, OutTextureMipColor)
	END_SHADER_PARAMETER_STRUCT()
...
IMPLEMENT_GLOBAL_SHADER(FDownsampleCubeFaceArrayCS, "/Engine/Private/ReflectionEnvironmentShaders.usf", "DownsampleArrayCS", SF_Compute);

#if RHI_RAYTRACING
class FRayTracingReflectionProbesRGS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FRayTracingReflectionProbesRGS)
	SHADER_USE_ROOT_PARAMETER_STRUCT(FRayTracingReflectionProbesRGS, FGlobalShader)
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, ShadowMaxNormalBias)
		SHADER_PARAMETER(int32, ReflectionEnableTwoSidedGeometry)
		SHADER_PARAMETER(FIntPoint, CaptureIndexRange)
		SHADER_PARAMETER(FIntPoint, CubeFaceRange)
		SHADER_PARAMETER(FIntVector, RayIndexOffsetScale)
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2DArray<float4>, CaptureColorOutput)
	END_SHADER_PARAMETER_STRUCT()
...
IMPLEMENT_GLOBAL_SHADER(FRayTracingReflectionProbesRGS, "/Engine/Private/RayTracing/RayTracingReflectionProbes.usf", "RayTracingReflectionProbesRGS", SF_RayGen);

static bool ShouldRenderRayTracingReflectionCaptures(const FViewInfo& View, const FScene& Scene)
{
	if ((CVarRayTracingReflectionsRayTraceEnvironmentCaptures.GetValueOnRenderThread() == 1) &&
		(Scene.ReflectionSceneData.SortedCaptures.Num() > 0))
	{
		// EHartNV ToDo : should force all effects turn on reflection capture ray tracing?
		return ShouldRenderRayTracingEffect(true);
	}

	return false;
}

void FDeferredShadingSceneRenderer::PrepareRayTracingReflectionCaptures(const FViewInfo& View, const FScene& Scene, TArray<FRHIRayTracingShader*>& OutRayGenShaders)
{
	if (!ShouldRenderRayTracingReflectionCaptures(View, Scene))
	{
		return;
	}

	auto RayGenShader = View.ShaderMap->GetShader<FRayTracingReflectionProbesRGS>();
	OutRayGenShaders.Add(RayGenShader.GetRayTracingShader());
}
#endif // RHI_RAYTRACING

void FDeferredShadingSceneRenderer::RenderRayTracingReflectionCaptures(
	FRDGBuilder& GraphBuilder,
...
#if RHI_RAYTRACING
{
	...
	if (!Scene->ReflectionSceneData.CubemapArray.IsValidForRayTracing())
	{
		return;
	}

	RDG_EVENT_SCOPE(GraphBuilder, "RayTracingReflectionCaptures");
	RDG_GPU_STAT_SCOPE(GraphBuilder, RayTracingReflectionCaptures);

	FRDGTextureRef CaptureColorTexture = GraphBuilder.RegisterExternalTexture(Scene->ReflectionSceneData.CubemapArray.GetRayTraceReflectionEnvs(), TEXT("RayTraceReflectionCapture"));
	const auto& Desc = Scene->ReflectionSceneData.CubemapArray.GetRayTraceReflectionEnvs()->GetDesc();
	FIntVector CubemapDims = Desc.GetSize();

	FRayTracingReflectionProbesRGS::FParameters* Parameters = GraphBuilder.AllocParameters<FRayTracingReflectionProbesRGS::FParameters>();
	Parameters->CaptureColorOutput = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(CaptureColorTexture));

	Parameters->TLAS = View.RayTracingScene.RayTracingSceneRHI->GetShaderResourceView();
	Parameters->ViewUniformBuffer = View.ViewUniformBuffer;
	Parameters->ReflectionCapture = View.ReflectionCaptureUniformBuffer;
	Parameters->LightDataPacked = View.RayTracingLightData.UniformBuffer;
	Parameters->LightDataBuffer = View.RayTracingLightData.LightBufferSRV;
	Parameters->SceneTextures = SceneTextures;
	Parameters->ShouldDoDirectLighting = Options.bDirectLighting;
	Parameters->ReflectedShadowsType = GRayTracingReflectionsShadows > -1 ? GRayTracingReflectionsShadows : (int32)View.FinalPostProcessSettings.RayTracingReflectionsShadows;
	Parameters->ShouldDoEmissiveAndIndirectLighting = Options.bEmissiveAndIndirectLighting;
	Parameters->ShadowMaxNormalBias = GetRaytracingMaxNormalBias();
	Parameters->ReflectionEnableTwoSidedGeometry = CVarRayTracingReflectionsEnableTwoSidedGeometry.GetValueOnRenderThread();
	Parameters->SSProfilesTexture = GraphBuilder.RegisterExternalTexture(View.RayTracingSubSurfaceProfileTexture); // TODO: should be converted to RDG
	Parameters->Forward = View.ForwardLightingResources->ForwardLightDataUniformBuffer;
	Parameters->ReflectionStruct = CreateReflectionUniformBuffer(View, EUniformBufferUsage::UniformBuffer_SingleFrame);
	Parameters->FogUniformParameters = CreateFogUniformBuffer(GraphBuilder, View);
	Parameters->CubeFaceRange = FIntPoint(0, 6);

	const bool bRayTraceSkyLightContribution = ShouldRayTracedReflectionsRayTraceSkyLightContribution(*Scene);

	// Fill Sky Light parameters
	FSkyLightData SkyLightData;
	SetupSkyLightParameters(GraphBuilder, Scene, View, bRayTraceSkyLightContribution, &Parameters->SkylightParameters, &SkyLightData);
	Parameters->SkyLightData = CreateUniformBufferImmediate(SkyLightData, EUniformBufferUsage::UniformBuffer_SingleDraw);
	SetupSkyLightVisibilityRaysParameters(GraphBuilder, View, &Parameters->SkyLightVisibilityRaysData);

	uint32 DisableSkyMask = CVarRayTracingReflectionsRayTraceEnvironmentCapturesDisableSkyMask.GetValueOnRenderThread();
	FVector SkyLightCaptureParametersValue = FVector::ZeroVector;
	if (Scene->SkyLight && !Scene->SkyLight->bHasStaticLighting && !DisableSkyMask)
	{
		...
		SkyLightCaptureParametersValue = FVector(1, Scene->SkyLight->SkyDistanceThreshold, 0);
	}

	Parameters->SkyLightCaptureParameters = SkyLightCaptureParametersValue;

	auto RayGenShader = View.ShaderMap->GetShader<FRayTracingReflectionProbesRGS>();
	uint32 NumSortedReflectionCaptures = Scene->ReflectionSceneData.SortedCaptures.Num();

	// Spread cubmap texel updates out over a grid, so that a cel is completely updated every N frames.
	uint32 DispatchWidth = CubemapDims.X;
	uint32 DispatchHeight = CubemapDims.Y;
	uint32 DispatchShift = CVarRayTracingReflectionsRayTraceEnvironmentCapturesTexelUpdateFrequency.GetValueOnRenderThread();

	if ((DispatchShift > 0) && (Scene->ReflectionSceneData.CubemapArray.CurrentTexelIndex >= 0))
	{
		int32 CelSize = 1 << DispatchShift;
		ensure(CelSize <= CubemapDims.X);

		DispatchWidth >>= DispatchShift;
		DispatchHeight >>= DispatchShift;

		int32 CurTexelOffset = Scene->ReflectionSceneData.CubemapArray.CurrentTexelIndex++;
		Parameters->RayIndexOffsetScale = FIntVector(CurTexelOffset % CelSize, CurTexelOffset / CelSize, 1 << DispatchShift);

		// Number of frames required to completely update cubemap
		int32 RequiredFrames = CelSize * CelSize;
		Scene->ReflectionSceneData.CubemapArray.CurrentTexelIndex = Scene->ReflectionSceneData.CubemapArray.CurrentTexelIndex >= RequiredFrames ? 0 : Scene->ReflectionSceneData.CubemapArray.CurrentTexelIndex;
	}
	else
	{
		// Update entire cubemap the first frame or if amortized updates are disabled
		Scene->ReflectionSceneData.CubemapArray.CurrentTexelIndex = 0;
		Parameters->RayIndexOffsetScale = FIntVector(0, 0, 1);
	}

	// Limit number of cubemaps updated per frame
	uint32 CubeUpdateLimit = CVarRayTracingReflectionsRayTraceEnvironmentCapturesCubeUpdateLimit.GetValueOnRenderThread();
	if ((CubeUpdateLimit > 0) && (Scene->ReflectionSceneData.CubemapArray.CurrentCubemapIndex >= 0))
	{
		uint32 CaptureStart = Scene->ReflectionSceneData.CubemapArray.CurrentCubemapIndex;

		// There may be fewer sorted captures this frame than there were last frame, reset if current index is invalid
		CaptureStart = CaptureStart < NumSortedReflectionCaptures ? CaptureStart : 0;

		uint32 CaptureEnd = CaptureStart + CubeUpdateLimit;

		if (CaptureEnd >= NumSortedReflectionCaptures)
		{
			CaptureEnd = NumSortedReflectionCaptures;
			Scene->ReflectionSceneData.CubemapArray.CurrentCubemapIndex = 0;
		}
		else
		{
			Scene->ReflectionSceneData.CubemapArray.CurrentCubemapIndex = CaptureEnd;
		}

		Parameters->CaptureIndexRange = FIntPoint(CaptureStart, CaptureEnd - CaptureStart);
	}
	else
	{
		// Update all cubemaps the first frame
		Scene->ReflectionSceneData.CubemapArray.CurrentCubemapIndex = 0;
		Parameters->CaptureIndexRange = FIntPoint(0, NumSortedReflectionCaptures);
	}

	GraphBuilder.AddPass(
		RDG_EVENT_NAME("RayTracingReflectionCapture"),
		Parameters,
		ERDGPassFlags::Compute,
		[RayGenShader, Parameters, &View, DispatchWidth, DispatchHeight](FRHICommandList& RHICmdList)
	{
		...
		RHICmdList.RayTraceDispatch(Pipeline, RayGenShader.GetRayTracingShader(), RayTracingSceneRHI, GlobalResources, DispatchWidth, DispatchHeight);
	});

	// Generate Mips
	uint32 CubeMipCount = Desc.NumMips;

	FDownsampleCubeFaceArrayCS::FPermutationDomain PermutationVector;
	TShaderMapRef<FDownsampleCubeFaceArrayCS> ComputeShader(GetGlobalShaderMap(FeatureLevel), PermutationVector);

	const ETextureCreateFlags RTCaptureArrayFlags = TexCreate_ShaderResource | TexCreate_RenderTargetable | TexCreate_UAV;

	FRDGTextureDesc MipsTextureDesc = FRDGTextureDesc::CreateCubeArray(CubemapDims.X, PF_FloatRGBA, FClearValueBinding::None, RTCaptureArrayFlags, NumSortedReflectionCaptures, CubeMipCount);
	FRDGTextureRef RTCaptureMipsTexture = GraphBuilder.CreateTexture(MipsTextureDesc, TEXT("RayTraceEnvMips"));

	for (uint32 MipIndex = 1; MipIndex < CubeMipCount; MipIndex++)
	{
		const uint32 MipResolution = 1 << (CubeMipCount - MipIndex - 1);

		FDownsampleCubeFaceArrayCS::FParameters* PassParameters = GraphBuilder.AllocParameters<FDownsampleCubeFaceArrayCS::FParameters>();
		...
		PassParameters->ArrayIndexRange = Parameters->CaptureIndexRange; 
		PassParameters->SourceCubemapArrayTexture = Scene->ReflectionSceneData.CubemapArray.GetRayTraceRenderTarget().ShaderResourceTexture;
		PassParameters->OutTextureMipColor = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(RTCaptureMipsTexture, MipIndex));

		FIntVector NumGroups = FIntVector::DivideAndRoundUp(FIntVector(MipResolution, MipResolution, 1), FIntVector(FDownsampleCubeFaceArrayCS::ThreadGroupSize, FDownsampleCubeFaceArrayCS::ThreadGroupSize, 1));

		// The groupd size per face with padding
		PassParameters->FaceThreadGroupSize = NumGroups.X * FDownsampleCubeFaceArrayCS::ThreadGroupSize;

		// We are going to dispatch once for all faces of all cubemaps
		NumGroups.X *= 6;

		// Dispatch with GenerateMips: reading from texture, writing into lower mip of identically sized texture through UAV, copying mips back
		...
		FRHICopyTextureInfo MipCopyInfo;
		MipCopyInfo.Size = FIntVector(CubemapDims.X >> MipIndex, CubemapDims.Y >> MipIndex, 1);
		MipCopyInfo.SourceSliceIndex = MipCopyInfo.DestSliceIndex = Parameters->CaptureIndexRange.X * 6;
		MipCopyInfo.NumSlices = Parameters->CaptureIndexRange.Y * 6;
		MipCopyInfo.SourceMipIndex = MipCopyInfo.DestMipIndex = MipIndex;
		AddCopyTexturePass(GraphBuilder, RTCaptureMipsTexture, CaptureColorTexture, MipCopyInfo);
	}
}
#else // !RHI_RAYTRACING
{
...
#if RHI_RAYTRACING
{
	...
	CommonParameters.MaxBounces = GetMaxReflectionBounces(View);
	...
	CommonParameters.RoughnessMultiplier = CVarRayTracingReflectionsRoughnessMultiplier.GetValueOnRenderThread();

	CommonParameters.AccumulateTime =  0; // set per pass later

	const int32 TimingMode = VisualizeRayTracingTiming(View) ? CVarRayTracingReflectionsTiming.GetValueOnRenderThread() : 0;
	const int32 TimeGatherPass = (TimingMode == 2 || TimingMode == 3);
	const int32 TimeShadePass = (TimingMode == 1 || TimingMode == 3);

	if (TimingMode > 0)
	{
		CommonParameters.CumulativeTime = GraphBuilder.CreateUAV(GraphBuilder.RegisterExternalTexture(SceneContext.RayTracingTiming));
	}
	else
	{
		CommonParameters.CumulativeTime = GraphBuilder.CreateUAV(OutDenoiserInputs->RayHitDistance); // bogus UAV to just keep validation happy as it is dynamically unused
	}
	...
	for (int32 SamplePassIndex = 0; SamplePassIndex < Options.SamplesPerPixel; SamplePassIndex++)
	{
		...
		for (uint32 PassIndex = 0; PassIndex < NumPasses; ++PassIndex)
		{
			...
			*PassParameters = CommonParameters;
			...
			if (DeferredMaterialMode == EDeferredMaterialMode::Gather)
			{
				PassParameters->AccumulateTime = TimeGatherPass;
				GraphBuilder.AddPass(
			...
			{
				PassParameters->AccumulateTime = TimeShadePass;
