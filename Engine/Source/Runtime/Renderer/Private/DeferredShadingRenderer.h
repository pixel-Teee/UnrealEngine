...
private:
	...
	*/
	...
	bool ShouldRenderDistortion(bool bNeedHybridTranslucencyDistortion=false) const;
	void RenderDistortion(FRDGBuilder& GraphBuilder, FRDGTextureRef SceneColorTexture, FRDGTextureRef SceneDepthTexture, bool bNeedHybridTranslucencyDistortion=false);
	...
	//EHartNV - ToDo double check if these are still valid
	void RenderViewTranslucency(FRHICommandListImmediate& RHICmdList, const FViewInfo& View, const FMeshPassProcessorRenderState& DrawRenderState, ETranslucencyPass::Type TranslucencyPass);
	...
	void RenderRayTracingReflectionCaptures(
		FRDGBuilder& GraphBuilder,
	...
	void RenderRayTracingTranslucencyView(
		...
		FRDGTextureRef* OutLayersColor,
		FRDGTextureRef* OutLayersDepth,
		int32 SamplePerPixel,
		int32 HeightFog,
		float ResolutionFraction);
	...
	static void PrepareRayTracingReflectionCaptures(const FViewInfo& View, const FScene& Scene, TArray<FRHIRayTracingShader*>& OutRayGenShaders);
	...
	/** BVH visualization */
	void RenderRayTracingVisualizeBVH(
		FRDGBuilder& GraphBuilder,
		TRDGUniformBufferRef<FSceneTextureUniformParameters> SceneTexturesUniformBuffer,
		FRDGTextureRef SceneColorTexture
	);
	void RenderRayTracingVisualizeBVHView(
		FRDGBuilder& GraphBuilder,
		const FViewInfo& View,
		TRDGUniformBufferRef<FSceneTextureUniformParameters> SceneTexturesUniformBuffer,
		FRDGTextureRef OutColorTexture
	...
	/** Ray Tracing timing visualization */
	void RenderVisualizeRayTracingTiming(FRDGBuilder& GraphBuilder, FRDGTextureRef SceneColorOutputTexture);
