...
#if RHI_RAYTRACING
int32 GRayTracingVolumeFogMode = 0;
FAutoConsoleVariableRef CVarRayTracingVolumeFogMode(
	TEXT("r.RayTracing.VolumeFogMode"),
	GRayTracingVolumeFogMode,
	TEXT("How to handle lights with volume fog when ray tracing shadows\n")
	TEXT("  0 - Do nothing, no volume shadows cast from ray traced lights (default)\n")
	TEXT("  1 - Compute shadow maps for lights attempting to shadow volumes\n")
	TEXT("  2 - Ray traced volume fog shadows"),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
#endif
...
int32 GetVolumetricFogRayTracingMode()
{
#if RHI_RAYTRACING
	return GRayTracingVolumeFogMode;
#else
	return 0;
#endif
}
...
static const uint32_t MaxRaytracedShadowedLocalInjectionLights = 16;
...
class TInjectShadowedLocalLightPS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_ARRAY(Texture3D, VoxelShadowInjectionInfo, [MaxRaytracedShadowedLocalInjectionLights])
	...
	class FRayTracingEnabled    : SHADER_PERMUTATION_BOOL("RAY_TRACING_ENABLED");
		...
		FGlobalShader(Initializer)
	{
		...
		VoxelShadowInjectionInfo.Bind(Initializer.ParameterMap, TEXT("VoxelShadowInjectionInfo"));
	}
...
public:
	...
	void SetVoxelShadowParameter(
		FRHICommandList& RHICmdList, 
		FRHITexture* VoxelShadowInjectionTexture)
	{
		FRHIPixelShader* ShaderRHI = RHICmdList.GetBoundPixelShader();
		SetTextureParameter(RHICmdList, ShaderRHI, VoxelShadowInjectionInfo, VoxelShadowInjectionTexture);
	}
...
private:
	...
	LAYOUT_FIELD(FShaderResourceParameter, VoxelShadowInjectionInfo);
};

IMPLEMENT_GLOBAL_SHADER(TInjectShadowedLocalLightPS, "/Engine/Private/VolumetricFog.usf", "InjectShadowedLocalLightPS", SF_Pixel);

class FVoxelShadowInjectionRGS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FVoxelShadowInjectionRGS)
	SHADER_USE_ROOT_PARAMETER_STRUCT(FVoxelShadowInjectionRGS, FGlobalShader)

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE(Texture3D, VoxelShadowInjectionInfo)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture3D, RWVoxelShadowInjectionInfo)
		SHADER_PARAMETER_SRV(RaytracingAccelerationStructure, TLAS)
		SHADER_PARAMETER_STRUCT(FLightShaderParameters, Light)
		SHADER_PARAMETER_STRUCT_REF(FVolumetricFogGlobalData, VolumetricFogParameters)
		...
		SHADER_PARAMETER(float, ZDispatchSize)
		SHADER_PARAMETER(FVector, LightPosition)
		SHADER_PARAMETER(uint32, bPointLight)
		SHADER_PARAMETER(uint32, bSpotLight)
		END_SHADER_PARAMETER_STRUCT()
...
IMPLEMENT_GLOBAL_SHADER(FVoxelShadowInjectionRGS, "/Engine/Private/RayTracing/RayTracingVoxelShadowRGS.usf", "VoxelShadowInjectionRGS", SF_RayGen);
...
bool LightNeedsSeparateInjectionIntoVolumetricFogForOpaqueShadow(const FLightSceneInfo* LightSceneInfo, FVisibleLightInfo& VisibleLightInfo)
{
		...
		&& LightProxy->CastsVolumetricShadow())
	{
		...
		const bool bUsesRayTracingVolumeShadows = ShouldRenderRayTracingShadowsForLight(*LightProxy) && GetVolumetricFogRayTracingMode() == 2;

		return ((GetShadowForInjectionIntoVolumetricFog(LightProxy, VisibleLightInfo) != NULL) || bUsesRayTracingVolumeShadows) || bStaticallyShadowed;
	}
	...
	FRDGTexture*& OutLocalShadowedLightScattering)
{
	...
	TArray<TTuple<const FLightSceneInfo*, FRDGTexture*>, SceneRenderingAllocator> LightShadowVolumes;

	bool bUseRayTracedVolumeShadows = IsRayTracingEnabled() && GetVolumetricFogRayTracingMode() == 2;
	...
	FRDGTextureDesc VoxelShadowInjectionDesc(FRDGTextureDesc::Create3D(VolumetricFogGridSize, PF_R32_FLOAT, FClearValueBinding::Black, Flags));
	...
	for (TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights); LightIt; ++LightIt)
	{
			...
			&& LightSceneInfo->Proxy->GetVolumetricScatteringIntensity() > 0)
		{
			...
			if ((View.ViewMatrices.GetViewOrigin() - LightBounds.Center).SizeSquared() < (FogInfo.VolumetricFogDistance + LightBounds.W) * (FogInfo.VolumetricFogDistance + LightBounds.W))
			{
				...
				bool bLightUsesRayTracingShadows = ShouldRenderRayTracingShadowsForLight(LightSceneInfoCompact);
				if (bUseRayTracedVolumeShadows && bLightUsesRayTracingShadows)
				{
					if (LightShadowVolumes.Num() < MaxRaytracedShadowedLocalInjectionLights)
					{
						FRDGTexture* VoxelShadowInjectionInfo = GraphBuilder.CreateTexture(VoxelShadowInjectionDesc, TEXT("VoxelShadowInjectionInfo"), ERDGTextureFlags::MultiFrame);
						
						// LightSceneInfo will be used as a key later to match shadow voxel textures to their appropriate lights
						LightShadowVolumes.Add(MakeTuple(LightSceneInfo, VoxelShadowInjectionInfo));
					}
				}
			}
		}
...
#if RHI_RAYTRACING
	if (bUseRayTracedVolumeShadows)
	{
		for (int32 LightShadowVolumeIndex = 0; LightShadowVolumeIndex < LightShadowVolumes.Num(); LightShadowVolumeIndex++)
		{
			const FLightSceneInfo* LightSceneInfo = LightShadowVolumes[LightShadowVolumeIndex].Get<0>();

			FRDGTextureUAV* VoxelShadowInjectionInfoUAV = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(LightShadowVolumes[LightShadowVolumeIndex].Get<1>()));

			FVoxelShadowInjectionRGS::FParameters* PassParameters = GraphBuilder.AllocParameters< FVoxelShadowInjectionRGS::FParameters>();
			PassParameters->VoxelShadowInjectionInfo = LightShadowVolumes[LightShadowVolumeIndex].Get<1>();
			PassParameters->RWVoxelShadowInjectionInfo = VoxelShadowInjectionInfoUAV;
			PassParameters->TLAS = View.RayTracingScene.RayTracingSceneRHI->GetShaderResourceView();
			PassParameters->ViewUniformBuffer = View.ViewUniformBuffer;
			PassParameters->VolumetricFogParameters = View.VolumetricFogResources.VolumetricFogGlobalData;
			...
			PassParameters->FrameJitterOffset = IntegrationData.FrameJitterOffsetValues[0];

			FForwardLightData& ForwardLightData = View.ForwardLightingResources->ForwardLightData;
			PassParameters->LightDirection = ForwardLightData.DirectionalLightDirection;

			FIntVector VolumetrixLightScatteringGroupSize(VolumetricFogLightScatteringGroupSizeX, VolumetricFogLightScatteringGroupSizeY, VolumetricFogLightScatteringGroupSizeZ);
			const FIntVector NumGroups = FIntVector::DivideAndRoundUp(VolumetricFogGridSize, VolumetrixLightScatteringGroupSize);

			int32 ZDispatchSize = NumGroups.Z * VolumetrixLightScatteringGroupSize.Z;
			PassParameters->ZDispatchSize = ZDispatchSize;

			PassParameters->LightPosition = LightSceneInfo->Proxy->GetPosition();
			PassParameters->bPointLight = LightSceneInfo->Proxy->GetLightType() == LightType_Point;
			PassParameters->bSpotLight = LightSceneInfo->Proxy->GetLightType() == LightType_Spot;

			FIntVector DispatchDims(NumGroups.X * VolumetrixLightScatteringGroupSize.X, NumGroups.Y * VolumetrixLightScatteringGroupSize.Y * ZDispatchSize, 1 /*NumGroups.Z * VolumetrixLightScatteringGroupSize.Z*/);

			TShaderMapRef<FVoxelShadowInjectionRGS> RayGenerationShader(GetGlobalShaderMap(FeatureLevel));
			ClearUnusedGraphResources(RayGenerationShader, PassParameters);

			GraphBuilder.AddPass(
				RDG_EVENT_NAME("VoxelShadowInjection Light%d", LightShadowVolumeIndex),
				PassParameters,
				ERDGPassFlags::Compute,
				[RayGenerationShader, PassParameters, DispatchDims, &View](FRHICommandListImmediate& RHICmdList)
			{
				...
				RHICmdList.RayTraceDispatch(Pipeline, RayGenerationShader.GetRayTracingShader(), RayTracingSceneRHI, GlobalResources, DispatchDims.X, DispatchDims.Y);
			});
		}
	}
	...
	if (LightsToInject.Num() > 0)
	{
		...
		for (int32 LightShadowVolumeIndex = 0; LightShadowVolumeIndex < LightShadowVolumes.Num(); LightShadowVolumeIndex++)
		{
			PassParameters->VoxelShadowInjectionInfo[LightShadowVolumeIndex] = LightShadowVolumes[LightShadowVolumeIndex].Get<1>();
		}
		...
		GraphBuilder.AddPass(
			...
			[PassParameters, &View, this, LightsToInject, VolumetricFogGridSize, GridZParams, bUseTemporalReprojection, IntegrationData, FogInfo, LightShadowVolumes, bUseRayTracedVolumeShadows](FRHICommandListImmediate& RHICmdList)
		{
			
			{
				TMap<FLightSceneInfo*, FVolumetricFogLocalLightFunctionInfo>& LocalLightFunctionData = View.VolumetricFogResources.LocalLightFunctionData;

				for (int32 LightIndex = 0, LightShadowVolumeIndex = 0; LightIndex < LightsToInject.Num(); LightIndex++)
				{
					...
					{
						bool bIsShadowed = LightNeedsSeparateInjectionIntoVolumetricFogForOpaqueShadow(LightSceneInfo, VisibleLightInfos[LightSceneInfo->Id]);
						bool bLightUsesRayTracedVoxelShadows = bUseRayTracedVolumeShadows && (LightShadowVolumeIndex < LightShadowVolumes.Num()) && (LightSceneInfo == LightShadowVolumes[LightShadowVolumeIndex].Get<0>());
						bool bUsesLightFunction = ViewFamily.EngineShowFlags.LightFunctions
							&& CheckForLightFunction(LightSceneInfo) && LightNeedsSeparateInjectionIntoVolumetricFogForLightFunction(LightSceneInfo);
						...
						PermutationVector.Set< TInjectShadowedLocalLightPS::FRayTracingEnabled >(bLightUsesRayTracedVoxelShadows);
						...
						if (bLightUsesRayTracedVoxelShadows)
						{
							PixelShader->SetVoxelShadowParameter(RHICmdList, LightShadowVolumes[LightShadowVolumeIndex].Get<1>()->GetRHI());
							++LightShadowVolumeIndex;
						}
...
class FVoxelShadowRGS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FVoxelShadowRGS)
	SHADER_USE_ROOT_PARAMETER_STRUCT(FVoxelShadowRGS, FGlobalShader)

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE(Texture3D, VoxelShadowInfo)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture3D, RWVoxelShadowInfo)
		SHADER_PARAMETER_SRV(RaytracingAccelerationStructure, TLAS)
		SHADER_PARAMETER_STRUCT(FLightShaderParameters, Light)
		SHADER_PARAMETER_STRUCT_REF(FVolumetricFogGlobalData, VolumetricFogParameters)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, ViewUniformBuffer)
...
IMPLEMENT_GLOBAL_SHADER(FVoxelShadowRGS, "/Engine/Private/RayTracing/RayTracingVoxelShadowRGS.usf", "VoxelShadowRGS", SF_RayGen);
...
class TVolumetricFogLightScatteringCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, VoxelShadowInfo)
	...
	TRDGUniformBufferRef<FSceneTextureUniformParameters> SceneTextures)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		RenderLocalLightsForVolumetricFog(GraphBuilder, View, bUseTemporalReprojection, IntegrationData, FogInfo, VolumetricFogGridSize, GridZParams, VolumeDescFastVRAM, LocalShadowedLightScattering);
			...
			bool UseRayTracedVoxelShadows = IsRayTracingEnabled() && GetVolumetricFogRayTracingMode() == 2;
#if RHI_RAYTRACING
			// Ray traced voxel shadows
			if (UseRayTracedVoxelShadows)
			{
				FRDGTextureDesc VoxelShadowDesc(FRDGTextureDesc::Create3D(VolumetricFogGridSize, PF_R32_FLOAT, FClearValueBinding::Black, Flags));

				IntegrationData.VoxelShadowInfo = GraphBuilder.CreateTexture(VoxelShadowDesc, TEXT("VoxelShadowInfo"), ERDGTextureFlags::MultiFrame);
				FRDGTextureUAV* VoxelShadowInfoUAV = GraphBuilder.CreateUAV(FRDGTextureUAVDesc(IntegrationData.VoxelShadowInfo));

				{
					FVoxelShadowRGS::FParameters* PassParameters = GraphBuilder.AllocParameters< FVoxelShadowRGS::FParameters>();
					PassParameters->VoxelShadowInfo = IntegrationData.VoxelShadowInfo;
					PassParameters->RWVoxelShadowInfo = VoxelShadowInfoUAV;
					PassParameters->TLAS = View.RayTracingScene.RayTracingSceneRHI->GetShaderResourceView();
					PassParameters->ViewUniformBuffer = View.ViewUniformBuffer;
					PassParameters->VolumetricFogParameters = View.VolumetricFogResources.VolumetricFogGlobalData;
					PassParameters->UnjitteredClipToTranslatedWorld = View.ViewMatrices.ComputeInvProjectionNoAAMatrix() * View.ViewMatrices.GetTranslatedViewMatrix().GetTransposed();
					...
					TShaderMapRef<FVoxelShadowRGS> RayGenerationShader(GetGlobalShaderMap(FeatureLevel));
					ClearUnusedGraphResources(RayGenerationShader, PassParameters);

					GraphBuilder.AddPass(
						RDG_EVENT_NAME("VoxelShadow"),
						PassParameters,
						ERDGPassFlags::Compute,
						[RayGenerationShader, PassParameters, DispatchDims, &View](FRHICommandListImmediate& RHICmdList)
					{
...

		{
			...
			PermutationVector.Set< TVolumetricFogLightScatteringCS::FRayTracingEnabled >(UseRayTracedVoxelShadows);
