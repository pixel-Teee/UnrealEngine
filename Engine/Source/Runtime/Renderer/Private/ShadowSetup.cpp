...
void FSceneRenderer::InitDynamicShadows(FRHICommandListImmediate& RHICmdList, FGlobalDynamicIndexBuffer& DynamicIndexBuffer, FGlobalDynamicVertexBuffer& DynamicVertexBuffer, FGlobalDynamicReadBuffer& DynamicReadBuffer)
{
	...
	TArray<FProjectedShadowInfo*,SceneRenderingAllocator> ViewDependentWholeSceneShadowsThatNeedCulling;
	{
		...
		for (TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights); LightIt; ++LightIt)
		{
			...
			const FLightOcclusionType OcclusionType = GetPreferredLightOcclusionType(*LightSceneInfoCompact.LightSceneInfo->Proxy);
			bool bNeedsVolumeLighting = false;
			if (GetVolumetricFogRayTracingMode() == 1)
			{
				auto LightProxy = LightSceneInfoCompact.LightSceneInfo->Proxy;
				if (//(LightProxy->GetLightType() == LightType_Point || LightProxy->GetLightType() == LightType_Spot || LightProxy->GetLightType() == LightType_Rect)
					 !LightProxy->HasStaticLighting()
					&& LightProxy->CastsDynamicShadow()
					&& LightProxy->CastsVolumetricShadow()
					&& LightProxy->GetVolumetricScatteringIntensity() > 0)
				{
					bNeedsVolumeLighting = true;
				}
			}
			if (OcclusionType != FLightOcclusionType::Shadowmap && !bNeedsVolumeLighting)
			{
				continue;
			}
