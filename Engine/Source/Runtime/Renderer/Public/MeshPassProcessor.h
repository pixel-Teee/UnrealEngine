...
namespace EMeshPass
{
	enum Type
	{
		...
		TranslucencyStandardRTRaster,
		TranslucencyAfterDOFRTRaster,
		TranslucencyAfterDOFModulateRTRaster,
		TranslucencyAllRTRaster,
...
inline const TCHAR* GetMeshPassName(EMeshPass::Type MeshPass)
{
	...
	case EMeshPass::DistortionRaster: return TEXT("DistortionRaster");		
	...
	case EMeshPass::TranslucencyRTMask: return TEXT("TranslucencyRTMask");

	case EMeshPass::TranslucencyStandardRTRaster: return TEXT("TranslucencyStandardRTRaster");
	case EMeshPass::TranslucencyAfterDOFRTRaster: return TEXT("TranslucencyAfterDOFRTRaster");
	case EMeshPass::TranslucencyAfterDOFModulateRTRaster: return TEXT("TranslucencyAfterDOFModulateRTRaster");
	case EMeshPass::TranslucencyAllRTRaster: return TEXT("TranslucencyAllRTRaster");
...
struct TMaterialProcessorShaders
{
	mutable TShaderRef<ShaderType> VertexShader;
	mutable TShaderRef<ShaderType> HullShader;
	mutable TShaderRef<ShaderType> DomainShader;
	mutable TShaderRef<ShaderType> PixelShader;
	mutable TShaderRef<ShaderType> GeometryShader;
	mutable TShaderRef<ShaderType> ComputeShader;
#if RHI_RAYTRACING
	mutable TShaderRef<ShaderType> RayHitGroupShader;
#endif

	TShaderRef<ShaderType> GetShader(EShaderFrequency Frequency) const
	{
...
using FMeshProcessorShaders = TMaterialProcessorShaders<FMeshMaterialShader>;
...
public:
	...
	template<typename ShaderType>
	void Initialize(TMaterialProcessorShaders<ShaderType> Shaders);
	...
	{
		//EHartNV ToDo : callables and miss shaders reuse the same slot in the shader bindings athit groups
		if (Frequency == SF_RayCallable || Frequency == SF_RayMiss)
		{
...
private:
	static int32 GatherBoundParameters(class FReadOnlyMeshDrawSingleShaderBindings& SingleShaderBindings, const int32 MaxUniformBuffers, const FRHIUniformBuffer** LocalUniformBuffers);

public:
	//EHartNV TODO: should these move to a binding writer too?
	void SetRayTracingShaderBindingsForMissShader(FRHICommandList& RHICmdList, FRHIRayTracingScene* Scene, FRayTracingPipelineState* Pipeline, uint32 HitGroupIndex, uint32 ShaderSlot) const;
#endif // RHI_RAYTRACING
...
struct TMeshProcessorShaders
{
	...
	template<typename ProcessorType = FMeshProcessorShaders>
	ProcessorType GetUntypedShaders()
	{
		ProcessorType Shaders;
