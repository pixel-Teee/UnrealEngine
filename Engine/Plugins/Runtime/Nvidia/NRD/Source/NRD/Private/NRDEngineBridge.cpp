...
#define MATERIAL_AWARE_DENOISING 1
...
// standard compilation environment adjustments for NRD shaders
void NRDModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
{
	// informs NRD to use the UE4 compatible resource definition style
	OutEnvironment.SetDefine(TEXT("COMPILER_UNREAL_ENGINE"), 1);

	//to enabled OCT-encoded normals
	OutEnvironment.SetDefine(TEXT("NRD_NORMAL_ENCODING"), MATERIAL_AWARE_DENOISING);
	OutEnvironment.SetDefine(TEXT("NRD_USE_MATERIAL_ID_AWARE_FILTERING"), MATERIAL_AWARE_DENOISING);
	OutEnvironment.SetDefine(TEXT("NRD_RELAX_MAINTAIN_MATERIAL_HISTORY"), MATERIAL_AWARE_DENOISING);
}

EPixelFormat NRDGetNormalRoughnessFormat()
{
#if MATERIAL_AWARE_DENOISING
	...
	const FNRDPackInputsArguments& InArguments)
{
	...
	Resources.NormalAndRoughness = CreateOutputTexture(NRDGetNormalRoughnessFormat(), TEXT("NRD.Input.NormalAndRoughness"));
